...
async def test_the_shim_does_not_resolve_past_the_venv(tmp_path: Path) -> None:
    ...
    does and a link cannot. the app repo's `test_project_env_e2e.py` proves the
...
async def test_the_cache_is_keyed_by_the_item_so_it_outlives_one_sandbox(tmp_path: Path) -> None:
    ...
    """
    sb = LocalProcessSandbox(root_dir=tmp_path / "sb", isolate=False)
    first = await sb.create(SandboxSpec(), item_id="item-a")
    again = await sb.create(SandboxSpec(), item_id="item-a")
    other = await sb.create(SandboxSpec(), item_id="item-b")
...
async def test_the_cache_is_swept_oldest_first_and_never_out_from_under_a_live_item(
    ...
    * oldest first, and `_exec_argv` stamps the directory on every exec so
    ...
    """
    sb = LocalProcessSandbox(root_dir=tmp_path / "sb", isolate=False)
    live = await sb.create(SandboxSpec(), item_id="live-item")
