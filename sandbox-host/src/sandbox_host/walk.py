...
def dict_lister(
    ...
    dirs: Iterable[str],
    version: Callable[[bytes], str] = lambda data: str(len(data)),
) -> ListDir:
    """A `list_dir` over an in-memory `{"/a/b.txt": bytes}` map plus a set of
    directory paths (the ones that may hold no files). Every ancestor of a
    file counts as a directory whether or not it was recorded."""
    ...
    def list_dir(rel: str) -> Iterable[Entry]:
        ...
        for name in sorted(children_of(rel, files)):
            data = files[f"{base}/{name}"]
            yield Entry(name, "file", len(data), version(data))
