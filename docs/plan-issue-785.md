...
**落地做法.** 小時粒度的計數是**建在天粒度之上**的(`hourColumns` 先問 `columnOf` 天數再補零頭,`instantAtHourColumn` 同樣先算整天再放回剩餘分鐘),所以那條已經正確的週末走法只有一份,兩個粒度不可能各走各的。原點正規化(週末 → 下一個工作日的第一小時)在 `workingOrigin` 一處完成,兩個方向共用。逆函式有 round-trip 測試,天/小時/含工時視窗各一組,並明確涵蓋負欄位與週末原點。新的 `dayBands` 也帶零寬保護 —— 就是為了這條前科。
