...
**落地做法.** 小時粒度的計數是**建在天粒度之上**的(`hourColumns` 先問 `columnOf` 天數再補零頭,`instantAtHourColumn` 同樣先算整天再放回剩餘分鐘),所以那條已經正確的週末走法只有一份,兩個粒度不可能各走各的。原點正規化(週末 → 下一個工作日的第一小時)在 `workingOrigin` 一處完成,兩個方向共用。逆函式有 round-trip 測試,天/小時/含工時視窗各一組,並明確涵蓋負欄位與週末原點。新的 `dayBands` 也帶零寬保護。

**這條風險還是發生了,而且是我守錯地方。** 上面那段只守住我**新寫**的走法。真正炸掉的是既有的 `monthTicks`:它自己手刻迴圈、沒有用 `advance()`,而且是唯一直接對**原始 minDate** 呼叫 `ymd` 的地方。P2 讓 minDate 可以帶時鐘之後,`ymd` 組出 `2026-01-05T09:30T00:00:00Z` = Invalid Date → NaN → `day >= visibleDays` 永遠不成立 → 純無窮迴圈,沒有錯誤、沒有記憶體成長,而且因為 auto-fit 會把長專案放進 month 區間,**開圖當下就凍**。對抗式 review 抓到的,不是我。

教訓有兩層。一是「新機制的前提改變時,要掃過**所有**舊消費端」——我只掃了自己碰過的。二是**凍住這種失敗根本測不到**:我為它寫的測試把整個 vitest worker 一起殺掉,連紅都報不出來。所以修法是兩層:`ymd` 走 `dayOf`(治因),外加給那個迴圈一個結構性上界(一個月不可能比一欄窄,所以 `visibleDays` 個月放不進 `visibleDays` 欄)。上界的價值不是防禦,是**把「凍住」換成「答案錯」**——錯答案 5ms 就紅,凍住永遠報不出來。
...
**最後採用的是排他上界,不是 23:59。** P2 先寫成「純日期的 end = 當天 23:59」,到 P3 算寬度時才發現那樣每個寬度都要再補一分鐘 —— 而那個 fudge 正是 P6 要刪掉的 `+1` 的同類。改成 `instantOf(end)` = 次日 00:00(排他)之後,寬度就是單純的 `end − start`,`+1` 和 `Never below 1` 兩個補丁一起消失,而「一天是 00:00–23:59」這句話依然成立 —— 它是同一件事從另一邊講。這是實作過程推翻自己前一個 phase 的設計,不是原計畫。
...
**驗證結果:名字騙人,但數量級沒問題。** `visibleDaysFor` 吃的是 `canvasWidth`,而 `canvasWidth = max(totalColumns × cpx, paneAvail)` —— 也就是**整個資料範圍**,不是視窗。所以「依可視範圍產生」這個前提是假的,現在就是假的,不是 P3 造成的。

但實際數字沒有暴增:fine row 有既有的 min-label 稀釋規則(`AXIS_MIN_LABEL_PX`),小時軸沿用同一條。一年、跳週末、07:00–21:00,在最細端(`PPD_MAX` = 576,欄寬 24px)是 261 個工作日 × 14 = 3654 欄,step 落在 2,所以 fine tick 約 1827 個、day band 261 個、加上每個 tick 一條 gridline,約 3900 個節點。同一個元件在天粒度畫一個十年專案(2610 個工作日)就已經是同一個量級,所以這不是新的一類問題,只是把既有的上限往上推。**沒有做虛擬化,也沒有假裝有做** —— 真要處理是獨立的一件事(#521 的清單虛擬化也還沒做)。
