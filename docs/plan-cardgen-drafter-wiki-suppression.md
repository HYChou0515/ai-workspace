...
- ⏸ **near-card 顯示撞到哪張既有卡**：刻意**延後**（後端 `ClusterMember` 尚無 target-card 欄，補一欄＋反查是獨立一段；P1 的分類計數已足夠讓使用者核對 P2 效果）。非本次 DoD 阻擋項。
...
- 🔴 **行為 DoD（live canned check，需 user 的 Ollama/1M 環境）**：真 LLM 跑 drafter 的證明照本檔案庫慣例是**app 內手動流程**（`scripts/check_cardgen_closed_loop_506.py` docstring 已更新指向 P1b funnel）：wiki-heavy collection 生成 → 待審核分頁看 D（抽草稿數）從 ~0 變多、wiki 解釋過但無卡的詞出現在提案。**此項待 user 在自己環境跑**（我無 Ollama，無法代跑）。

**P3 —（視 P1 觀測結果）覆蓋補洞**
- ⏸ **刻意未做**：條件是「P1 摘要顯示 `n_units << 選取數`」，需 user 的 live funnel 數字才能判斷。plan 原文即「先不預設要做——用 P1 的數字決定」。P1b 已把 `n_units` 曝出，等 user 回報數字再決定。
