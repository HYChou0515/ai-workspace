...
2. ~~**批次大小上限**~~ —— 已做,而且**每條路各自綁自己的請求,綁在無界的那個東西所在的
   地方**:`files/facade.py` 的 `BATCH_PATHS` 管 HTTP body,`filestore/specstar_impl.py` 的
   `_QUERY_PATHS` 管 SQL `IN`。刻意**不做成 config 旋鈕**(新旋鈕要記 migrations 帳)。
   ⚠️ 第一版只有 sandbox 那條有分塊,冷路徑把整串路徑塞進一句 SQL `IN` —— 跟
   `kb/graph/link.py` 的 `PAGE = 500` 同一類問題(4 萬個 id 組出 937KB 的 statement 被資料庫
   拒絕)。**加第二個批次原語時沒沿用已經在樹上的規則。**
   ⚠️ 第二版把上限提到共用的 `read_all` 裡,看起來更整齊,**但是錯的**:那會讓一次讀取每
   200 個路徑就重解析一次活性,而 sandbox 若在讀到一半被回收,前幾塊由 sandbox 回答、其餘
   由 durable 快照回答 —— 一次讀取跨了兩個 store,正是 `_read_with` 存在要防的事。
...
4. **記憶體:一個型別的記錄現在會一次全部進記憶體**(使用者拍板:接受,但記下來)。
   `_parse_type` 要跨 chunk 累積才能一次回傳,master 則是讀一筆、解析一筆、丟一筆。
   68 個 issue 的 markdown 不痛;幾千筆、或記錄裡塞了大附件就會痛,而且**它跟資料量一起長**。
   不做的理由:到目前為止所有量到的痛都是「來回次數」,記憶體是推測的 —— 拿猜的東西改設計
   正是這份計劃一開始要避免的事。真的要處理,做法是讓 `_parse_type` 邊拿邊解析、不留 bytes
   (要改回傳形狀,會擴散到呼叫端)。
5. **`_warm` 仍把「端點不存在」當成該重建 sandbox 的訊號**。`0082a9e9` 只改了訊息(依使用者
   指示,不加 fallback),例外型別沒動 —— 而重建不可能生出一個端點。這是行為改變不是訊息,
   所以留著;要動的話是另一個決定。

6. **嚴格讀取不再「碰到第一個缺失就停」**(review F3,接受)。舊版在 chunk 迴圈裡就丟例外;
   現在 facade 一律寬容讀完,再由 `read_all` 判斷。實測 450 個路徑、缺失落在第一塊:批次
   從 1 次變 3 次(熱路徑),冷路徑從 `[200]` 變 `[200,200,51]`。
   接受的理由:它只在**最後會丟例外**的那條路上多花,而換到的是「嚴格與否只寫一次」——
   兩個能力並存正是第四個近失躲藏的地方。但這是一個在乎來回次數的分支上**增加**來回,
   所以寫下來。
7. **`FileNotFound` 的訊息掉了 `ws:` 前綴**(review F4,接受)。`api/app.py` 會把
   `str(exc)` 原封不動渲染進 404 的 body,所以這是使用者看得到的變化。
   接受的理由:改之前訊息形狀是**看路徑決定**的(facade 路線給 `/r/x.md`,原始 store 路線給
   `ws1:/r/x.md`),現在兩條都給路徑 —— 比原本一致,而且對使用者可讀的 body 來說更好。
