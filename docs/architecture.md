...
- `SourceDoc`：一份上傳文件。natural-key id = `{collection}/{created_by}/{path}`；
  `content` 為原始 bytes（`content.file_id` = xxh3 內容雜湊，dedup 用）；`status` =
...
natural-key id 的 `SourceDoc`（`status=indexing`），**同步且快**。慢的 `index()`（切塊 →
嵌入 → 建 `DocChunk` → `status=ready`/`error`）丟到 **FastAPI BackgroundTask** 跑，不擋上傳
回應。資料夾上傳＝把每個檔案以其相對路徑當檔名上傳（等同於解一個壓縮檔）。
...
citations 不在串流裡——FE 在 `done` 後 refetch thread 取得已持久化的 `[n]`。RCA agent 透過
`ask_knowledge_base` 工具（`AgentToolContext.ask_kb`，由 `create_app` 注入）呼叫 KB agent，
拿回一段含來源的整合答案。
