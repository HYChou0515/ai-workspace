...
- `SourceDoc`：一份上傳文件。id = natural key `{collection}/{created_by}/{path}` 編碼成
  slash-free 的不透明 token（`encode_doc_id`；永不解析）；`content` 為原始 bytes
...
**opaque slash-free id**（`encode_doc_id`＝natural key `{collection}/{user}/{path}` 百分比編碼；
specstar id 不能含 `/`，見 `kb/doc_id.py`）的 `SourceDoc`（`status=indexing`）。`store` 與慢的
...
| SourceDoc id ＝ natural key 編碼成 slash-free 的不透明 handle | specstar id 不能含 `/`；id 永不解析，欄位/`created_by` 才是真相，render 用 query param |
