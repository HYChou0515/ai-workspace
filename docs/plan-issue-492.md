...
- **P11 · 症狀 3（idle 慢回無 streaming）** — ops + 小 code
  ...
  - **ops**：`SANDBOX_HOST_IDLE_TTL` 拉到 > app 的 8h（或 `0`）；讓「當機備援」回到它該有的角色。
  - **code（可選）**：restore 期間 emit 進度事件（`restore` 加 `on_progress`，經 `ensure_handle` 傳 turn 的 sink），前端顯示「還原中 N/M」而非空白 running 卡片。P4 的 host-rsync restore 讓這步更快，進度事件是錦上添花。
