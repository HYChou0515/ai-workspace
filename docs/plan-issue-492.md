...
- **P4 · app 接線（含(1)讀取修法）** ✅
  - **讀取修法（實作結果，優於原構想）**：durable 改成 NFS 樹（**#501 後 `sandbox.durable.kind: nfs_tree`**，原為全域 `filestore.kind`）後，facade `_warm` 對非擁有者 pod（HTTP `handle_for_id`→None、無 session）回 None → 直接讀**本機掛載的 NFS 樹**（host persist 保持 ≤30s 新鮮）；擁有者 pod 仍讀自己的 live sandbox。這一步同時堵掉「永遠 stale 的 specstar snapshot」與跨 pod HTTP hang。
...
- **P6 · quota + GC 收尾** ✅（by construction）
  #245 workspace quota 改 `du` on `{nfs}/{item}/`（取代 specstar `Sum` aggregate）——已由 P1 `NfsTreeFileStore.workspace_usage`（du）+ P2 `MigratingFileStore.workspace_usage`（legacy∪primary `stat_all`）交付，facade duck-type 之，含單元測試。**退役 workspace blob GC**：`nfs_tree` 下 workspace 檔改落 NFS 樹、不再寫成 specstar blob，故 `spec.gc(reconcile)` 掃不到*新的* workspace blob（emergent no-op，無碼可刪）；M2 期間 legacy blob 由 `SpecstarFileStore` 繼續保護（正確，遷移完成前不可退役）。KB/wiki 的 blob GC 完全不動。**（#501 修正）**：API 的 specstar filestore 現在**永遠**被建構（`get_filestore` 與 sandbox 的 `nfs_tree` durable 分離），所以 `WorkspaceFile` 模型一律註冊、blob GC 一律完整——sandbox 選 `nfs_tree` 不再讓 specstar filestore 消失。
