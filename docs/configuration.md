...
collection 內、樹的順序 = 你在文件樹看到的順序，資料夾在前、檔名自然排序），檔案交界處會標一行檔名，
模型看得出跨了檔。補進來的文字是 **rerank 排序和模型閱讀**的對象；引用 `[n]` 仍然指向命中的那一段，
引用卡片的摘錄和文件頁的 highlight 不會變成一大坨。

| 設定 | 效果 |
| --- | --- |
| `2000`（預設） | 中英文各約多帶一塊 chunk（英文一塊約 1,800 字元、修過中文切段後的中文也接近） |
| `0` | 關掉，行為與此版之前逐位元相同 |
| 更大 | rerank 的 prompt 等比例變長（它是 listwise，一次把所有候選塞進同一個 prompt） |
