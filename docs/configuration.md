...
| **讓頁面自己排的工作真的會跑** | `server.trigger_check_interval_sec`（掃描間隔秒數，**預設 `0` = 完全不掃**）。這一顆同時管工程師寫的 `triggers.json` 和頁面寫的 `schedules.json`——**沒開的話兩者都不會動，而且不會有錯誤訊息**。這個數字就是「最晚會遲到多久」；60 是合理起點。多 pod 時它也是**掃描的 window**：每個 window 只有一顆 pod 真的去讀 `triggers.json` / `schedules.json`（同一份 window ledger 上多一列 `__scan__:*` 的 claim），其餘 pod 那一 tick 什麼都不做；贏家半路死掉只損失那個 window，下個 window 重新選——所以「最晚遲到一個 interval」的承諾不變（#804） |
...
| `chat_video` | Export ▾ → 影片的伺服端上限（像素、秒數、輸出大小、進度心跳）；worker pod 的 memory limit 是照它們量的 | 少改（改了要重量 [chat-video.md](chat-video.md#要多少資源量的41-秒的範例1080p)） |
...
帳仍然記在 item 的 `owner` 上。維運面要知道的:

- **選的是上限,不是預約。** 選了 4 核不會當下扣額度——只有 sandbox 活著才算,跟今天完全
  一樣。額度的帳是從心跳推導的,不是 create+1 / kill−1 的計數器。
- **預設 `min(App 上限, 個人額度)`,而且不落地。** 欄位空 = 沒人指定。調整某人的額度之後,
  他那些沒特別設定的 item 自動跟著變。
- **沒設 `per_user` 的部署看不到那半個面板。** 沒有額度就沒有「花費」這回事,畫 `0 / 0`
  是雜訊。但「環境在跑嗎、關掉它」那半邊**永遠都在**——那是關於機器,不是關於額度。
- **後端套不了 cgroup 時不給桿子**,並顯示「無法確認」。刻意不說「未強制」:`HttpSandbox`
  對「主機連不上」和「真的什麼都不限」回報同一個值,聲稱分得出來就是無中生有。
- **`change_permission` 現在可以從分享對話框授出去**(後端一直支援,UI 以前沒開)。它跟角色
  清單**分開呈現**,因為被授權的人可以再把這個 item 轉授給任何人,而且可以決定花掉 owner
  多少額度。
- **撞到額度的拒絕訊息會列出是哪些 item 佔著、就地可關**——但只對 owner 自己。協作者撞到
  owner 的額度時只會知道原因,不會看到那個人其他 item 的標題。

⚠️ 帳仍然記在 `owner` 欄位上,而它到今天還是任何有寫入權的人一個 PATCH 就能改掉
(issue #687)。整條規則的強度上限在那裡,這是知情的取捨。
...
- `window` 跨午夜是常態，不是邊角；`timezone` 一定要寫 IANA 名稱，**別依賴容器的時鐘**——base image 換了你的下班窗會跟著漂，而且沒人查得到。窗沒設或打錯、時區名稱無效 → 下班長跑**關閉**（不會亂猜一個時段,也不會讓每 60 秒的掃描迴圈爆掉），聊天面板上那個勾選框會直接顯示停用。
- `offhours.max_rounds` 是**累計**的:一個 goal 花掉就沒了,所以跨三個晚上的任務跑得完,又不會變成每晚無限續命。上班的 `max_rounds` 是**另一個**計數器,兩者分開算。
- **四種結局各自有交代**:達成 / 卡住(連續 2 輪沒進展就停下等人,不會把額度燒完)/
  額度用盡 / 窗關了(還沒做完,今晚接著跑)。**過夜跑過的**才會在對話寫一則交班摘要
  (一次便宜模型呼叫)+ 響鈴通知;白天你自己看著跑完的不會通知——為看得到的事情
  響鈴只會訓練你忽略下一則。
...
failover:
  ...
  rate_limit_budget_s: 7200  # 例外：429（rate limit）不切線、在原 endpoint 等它聲明的窗口，
                             ...
                             # 設 0 = 不等，429 一律停用該線並切換（純 failover 行為）。
                             # 只影響 agent 對話鏈；KB 檢索鏈遇 429 本來就直接停用＋切換，設了無效。
```
