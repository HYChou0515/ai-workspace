...
| `onboarding` | 進 App 時彈的歡迎卡（#161）：`{version, title, intro, points: [{title, body}], footer}`。**`intro`、每個 `body`、`footer` 是 markdown**（GFM + `$…$` 數學，跟聊天訊息同一條管線；沒有 raw HTML）；`title` 是純文字。`footer` 選用，畫在條列之下、按鈕之上。要放截圖就把圖放 `assets/`、在 `body` 或 `footer` 寫 `![說明](assets/x.png)`（純檔名、png/svg/jpg/jpeg/webp/gif；絕對路徑與 `https://` 原樣通過，其他相對路徑不會被猜、會壞得看得見）。**教學內容改了就手動 bump `version`**——每個使用者會再看到一次，按「永遠不顯示」後不再出現；不要跟發版版本綁在一起。`_template/app.json` 有寫好的範例 |
...
- **這個數字現在是「天花板」,不是「固定值」。** 使用者可以在 item 頁面把**這一個** item
  的環境調得更小(見下),但永遠不能調得比你在這裡宣告的更大。所以宣告時想的是
  「這個 App 的 item **最多**可以吃多少」,不是「每個 item 都要吃多少」。
...
item 頁面的「環境」面板讓人替**那一個 item** 指定 cpu / 記憶體,由 item 的 `owner` 的額度
支出。三件跟 App 作者有關的事:

- **預設是 `min(你宣告的上限, 那個人的額度)`**,而且**不會被存起來**——欄位空著就代表
  「沒人指定」,每次都重新計算。所以管理員調整某人的額度之後,他那些沒特別設定的 item
  會自己跟著變,不會停在三個月前算出來的數字。
- **取小的那一個是必要的,不是保守。** 只取 App 上限的話,「App 上限 4 核 + 那個人額度
  2 核」會產生一個**永遠開不起來**的 item:准入控制是 `incoming > limit` 就拒絕,關掉其他
  東西也沒用。
- **要調大小需要 `change_permission`**,不是 `write_meta`。因為它花的是 owner 的額度,而且
  那是 agent **永遠拿不到**的動詞——這個 item 的 agent 就跑在那個 sandbox 裡,用一個 AI
  拿得到的動詞等於讓它自己調高自己的上限。
