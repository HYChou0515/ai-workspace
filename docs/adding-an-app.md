...
| `onboarding` | 進 App 時彈的歡迎卡（#161）：`{version, title, intro, points: [{title, body}], footer}`。**`intro`、每個 `body`、`footer` 是 markdown**（GFM）；`title` 是純文字。`footer` 選用，畫在條列之下、按鈕之上。要放截圖就把圖放 `assets/`、在 `body` 或 `footer` 寫 `![說明](assets/x.png)`（純檔名、png/svg/jpg/webp/gif；絕對路徑與 `https://` 原樣通過，其他相對路徑不會被猜、會壞得看得見）。**教學內容改了就手動 bump `version`**——每個使用者會再看到一次，按「永遠不顯示」後不再出現；不要跟發版版本綁在一起。`_template/app.json` 有寫好的範例 |
