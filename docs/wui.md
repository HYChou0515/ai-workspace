...
- **不會 rebuild。** 開頁不叫醒 sandbox、不跑 build；讀者拿到的是資料夾**現在**的成品——
  最近一次 build 的結果，不論那次是 Deploy 還是 Rebuild、還是別人打開時的 Auto-rebuild
  建的；沒有 build 的頁面則是 `index.html` 現在的樣子，存檔就生效。讀者不建，發布者建——
...
一個正被人拿著連結看的頁面，工作區裡任何一次 build 都會**立刻**換掉他眼前的東西——
要在別人面前動手，先複製一份資料夾（見下方「沒有快照」）。
