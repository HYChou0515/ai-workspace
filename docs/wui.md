...
這頁的人」——也不是拿連結的人，他們看到的永遠是上一次 Deploy 建好的成品（見下一節）。
...
每一頁都有自己的網址——`/w/{app}/{item}/{view 檔的路徑}`——打開就是這一頁本身，
**沒有工作區的外殼**：沒有導覽列、沒有檔案樹、沒有麵包屑。跟著連結來的人無處可去，
那些東西對他只是噪音。

工具列最右邊有一顆 **Deploy**：

- 有 build 的頁面會**先 rebuild，成功了才把網址端出來**——所以網址指到的一定是剛建好
  的成品，不會是舊的 `dist/`。沒有 build 的頁面直接給網址。build 失敗就不會出現
  「Deployed」，改成一句「Deploy failed」壓在 build 輸出上面，什麼都不端出來。
...
- **不會 rebuild。** 開頁不叫醒 sandbox、不跑 build；讀者拿到的就是上次 Deploy 建好的
  成品。發布者建、讀者不建——這是 Deploy 為什麼一定要先 build 的原因。
- 沒有工具列、沒有 build 輸出、沒有 reports。
- 頁上的按鈕（tool、workflow）**照常能按**——那是這頁存在的理由。sandbox 只在真的按了
  才會被叫醒。
- 沒建過的頁面會看到「This page has not been published yet.」，不是空白。

所以：改了 `src/` 之後要讓拿連結的人看到新版，**就再按一次 Deploy**。Auto-rebuild
只保護在工作區裡打開這頁的人。
