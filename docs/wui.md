...
按 app 分組、最新 Deploy 的在前。每一列是頁面的圖示（一個圓圈，見下）、頁面的名字
（點了在新分頁開讀者版）、它所在的 item（點了進 item 的工作區）、誰在什麼時候 Deploy 的、
一顆星（我的最愛，見下）。這是頁面「被找到」的唯一地方：一頁做好了但沒按 Deploy，只有拿到
連結的人知道它存在。
...
- **我的最愛。** 每一列右邊有一顆星，按了這一頁就多列在總覽最上面的「我的最愛」群組（在
  ...
  ，不上伺服器、別人看不到；能開頁面就能加星，不需要能 Deploy。標了星的頁面被 Remove 或
