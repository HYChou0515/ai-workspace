...
| `src/workspace_app/agent/tools.py` | 整個 function-tool 目錄：每個 `*_impl` coroutine（含 #323 opt-in 的 `save_workflow_impl`）、`_IMPLS` 名稱→impl 註冊表、`_WORKSPACE_TOOLS`（RCA 預設集）、把 allowed-list 變成 `agents.FunctionTool` 的 `build_tools()`（含條件式 `read_skill` 注入）、`builtin_tool_descriptions()`（內建 tool 名→其 LLM-facing description，餵 #322 tool catalog 的單一來源）。也含 exec/read 格式化輔助（`_format_exec`/`_truncate_middle`）與 `infer_modules` 的 fan-out 機制。 |
| `src/workspace_app/agent/tool_authz.py` | #309 的 agent-tool 授權漏斗：`TOOL_VERBS`（每個 item-level tool 行使**哪些** verb——讀又寫的工具要兩個都列,ceiling 取聯集)、`ceiling_from_tools()`（由 preset 的 allowed tools 導出 AI 的 verb ceiling）、`LEGACY_TOOL_RENAMES`（stored `allowed_tools` 的舊名→現名正規化；`build_tools` 與這裡的 ceiling 走同一份,否則會發出一個註冊得到卻永遠被拒的 tool)、以及 `authorize_tool()`——**`TOOL_VERBS` 表裡的**每個 tool impl 在碰工作區之前呼叫它(表外仍有沒過閘的工具,清單見該模組 docstring),以 `Actor.ai(ceiling ∩ speaker,含 speaker 的 groups)` 對 item 的 `permission` 求值。拒絕時留下一行 WARNING（verb / item / user / owner / visibility / speaker 的 group 數(**這次判斷**沒用到就是 `n/a`——被 ceiling 擋下時不會去問) / 是否落在 ceiling 內),模型只拿到一句話。 |
...
- `src/workspace_app/agent/tools.py` — `_IMPLS` 註冊表、`_WORKSPACE_TOOLS`、`build_tools`，以及 `kb_search_impl` / `ask_knowledge_base_impl` / `infer_modules_impl` / `save_workflow_impl` + `_profile_tool_ceiling`（#323）/ `builtin_tool_descriptions`（#322）/ `_format_exec` / `_truncate_middle`。
