...
| `web/src/renderers/entity/viewKindRegistry.tsx` | view-kind 註冊表——`*.ai.yaml` 的 `view:` → Component。**唯一**知道哪些 kind 名稱已被佔用的地方：已註冊的（`registry`）+ 容器自己接手的保留名（`RESERVED`，目前只有 `health`）。`parseViewSpec` 不再持有副本，它只判斷「是不是 view 檔」。`registerViewKind` 開放第二方註冊——runtime view plugin（#847/#848，`web/src/viewPlugins/loader.ts` 在第一次 render 前 `import()`）或編譯期的 `web/src/ext/`（#698），內建 kind 走同一個函式、沒有特權路徑，撞名或撞保留名都直接丟例外。 |
