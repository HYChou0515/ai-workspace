...
| `web/src/components/ToolsPickerModal.tsx`、`ToolsChecklist.tsx`、`lib/toolGroups.ts`、`toolCatalog.tsx` | #322 per-App 工具挑選器：`AgentHeader` Tools 按鈕 → `ToolsPickerModal` → `ToolsChecklist`（每列 Default／On／Off 三態、搜尋、reset；`plan-tools-picker-groups`：列按伺服端給的 `group` 折疊——內建一律收在一組、名稱就叫 `builtin`，套件的授權指令各一組；組標題列有同樣的三態，狀態由組內列**導出**（全同就是那一態、否則「混合」三個都不亮），按下去改寫組內每一列，不另外儲存；組只涵蓋天花板內的列，所以只授兩個指令的套件兩列都開就是開啟；預設收合、混合或搜尋命中才自動展開；單列的組就是那一列——規則全在純函式 `lib/toolGroups.ts`），經 read-modify-PUT `setField('attached_tool_prefs', …)` 存回（`WorkspaceShell`→`ItemChatShell`→`AgentPanel`→`AgentHeader`）；`toolCatalog.tsx` 讀 `GET /tools` 為未映射工具補標籤（i18n 疊在上層）。query key `qk.itemTools`／`qk.toolsCatalog`。 |
