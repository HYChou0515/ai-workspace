...
| `web/src/renderers/entity/viewKindRegistry.tsx` | view-kind 註冊表——`*.ai.yaml` 的 `view:` → Component。**唯一**知道哪些 kind 名稱已被佔用的地方：已註冊的（`registry`）+ 容器自己接手的保留名（`RESERVED`：`health` 與 `wui`）。`parseViewSpec` 不再持有副本，它只判斷「是不是 view 檔」。`registerViewKind` 開放第二方註冊——runtime view plugin（#847/#848，`web/src/viewPlugins/loader.ts` 在第一次 render 前 `import()`）或編譯期的 `web/src/ext/`（#698），內建 kind 走同一個函式、沒有特權路徑，撞名或撞保留名都直接丟例外。 |
| `web/src/renderers/entity/ViewErrorBoundary.tsx` | 全 repo **唯一**的 error boundary，包住整個 view 面板（含 header——`spec.title` 正是原本的崩潰點）。第二方程式碼一個 throw 本來會把整個 app 卸載成白畫面（#698）。`resetKey`=view 檔路徑+內容；plug-in 讀的**其他**檔案它看不到，所以另有 Retry。 |
| `web/src/renderers/entity/public.ts` | 第二方 view kind 的**唯一**入口面（註冊 + 檔案存取 + 繪圖 helper + `useSandboxRun` / `viewDocument` / `SDK_VERSION`）。runtime plugin 經 import map 以 `@aiws/view-sdk` 取得它（`web/vite-plugins/sharedModules.ts`）；`src/ext/**` 與 `view-plugins/*/web/src` 只能從這裡 import，由 `src/ext/imports.test.ts` 強制。 |
...
| `web/src/components/ToolsPickerModal.tsx`、`ToolsChecklist.tsx`、`lib/toolGroups.ts`、`toolCatalog.tsx` | #322 per-App 工具挑選器：`AgentHeader` Tools 按鈕 → `ToolsPickerModal` → `ToolsChecklist`（每列 Default／On／Off 三態、搜尋、reset；`plan-tools-picker-groups`：列按伺服端給的 `group` 折疊——agent 本體的函式一律收在一組「核心工具／Core tools」（fold id `builtin`），套件的授權指令各一組；組標題列有同樣的三態，狀態由組內列**導出**（全同就是那一態、否則「混合」三個都不亮），按下去改寫組內每一列，不另外儲存；組只涵蓋天花板內的列，所以只授兩個指令的套件兩列都開就是開啟；開或收只在 modal 開啟時（那時混合的組開）與搜尋詞改變時（命中的組開、親手收過的釋放、親手開過的保留、混合集合重取當下）決定，中間只有使用者的點擊會動它；搜尋中組就是它命中的那幾列（項數／狀態／組三態／reset 同一集合）；單列的組就是那一列。分組、順序、狀態導出、整組改寫在純函式 `lib/toolGroups.ts`，開合與搜尋在元件裡；**列的粒度由伺服端決定**：整包授權的套件在 route 端已展開成一個指令一列（`expand_entries`；`effective` 讀自和 turn 同一個定案 config），前端只照 `group` 折；`overrideFromTools` 從列重建 override，所以舊的整包鍵在第一次（有改動的）Save 時自然變成逐指令鍵），經 `setField('attached_tool_prefs', …)`（JSON-Patch `replace` 整個欄位）存回（`WorkspaceShell`→`ItemChatShell`→`AgentPanel`→`AgentHeader`）；`toolCatalog.tsx` 讀 `GET /tools` 為未映射工具補標籤（i18n 疊在上層）。query key `qk.itemTools`／`qk.toolsCatalog`。 |
| `web/src/components/SkillsModal.tsx`、`api/workspaceSkills.ts` | #298+#380 技能面板：`AgentHeader` Skills 按鈕 → `SkillsModal`，讀 `api.getItemSkills`（`GET …/items/{id}/skills` → `qk.itemSkills`）列出**三來源**（shared／profile／workspace）每列一個 source badge。每列（1）Default／On／Off 三態，Save 經 read-modify-PUT `setField('attached_skill_prefs', …)` 存回（threading 仿 tool picker，多一條 `onSaveSkillPrefs`）；（2）**Apply** 鈕把該 skill 排進**這回合**（`onToggleApply`），狀態 lift 到 `AgentPanel`，在 composer 顯示 accent 色 chip（有別於 @mention／圖片／suggestion chip），送出時經 `send(content, {applySkills})` 帶走並自動清空——KB chat 不帶。workspace skill 另有下載整包 `.skill/<name>/` zip ＋資料夾匯入（沿用 #298 的 file 路由）。 |
...
| `ViewRenderer` 註冊表（可註冊） | 開放式 dispatch table + plug-in seam | `web/src/renderers/entity/viewKindRegistry.tsx` | 內建 `table`/`board`/`gantt`；第二方 runtime plugin（範例 `view-plugins/csv-table`）或 `web/src/ext/*` |
