...
API 觸發、headless、profile 層級的 workflow 引擎：一個 profile 只要宣告了 workflow,就能被跑起來 — 撰寫者可選 trusted Python `run.py`(編排碼),或一份宣告式的 `workflow.json`(#323 的「降級 DSL」,由 trusted interpreter `build_run` 跑在**同一批** step 原語上;JSON 是資料,API 裡不跑使用者程式碼)。核心觀念是 **filesystem-as-journal** — 每個 step 都把 `{input-hash, result}` 寫進 `/.workflow/<workflow_id>/step_<name>/<key>.json`,所以重跑一個 run 會直接跳過仍然有效的前綴(不重打 LLM/sandbox/chat),而不是抽象地 replay。
...
| `src/workspace_app/workflow/handle.py` | `WorkflowHandle`(`wf`)— run 對自己 workspace 的視角。FileStore 的 async 薄包裝(read/write/glob/exists/delete + `journal_dir`)、journaled 能力節點(`ingest_to_collection`/`upsert_context_card`/`convert`(#324)/`find_overwrite_card`)、`map()` 平行 for-each,並帶 driver 接上的 leaf driver 與 run-scoped `credential`。 |
| `src/workspace_app/workflow/discovery.py` | 用 **file path**(非 `import_module`)載入 profile 的 `run()`(與選用的 `preflight`,#283)。處理 `workflows: [...]` list 版面 vs legacy 單一 root `run.py`,`validate_workflow_profiles` 在啟動時對缺 `run()`/重複或空 id fail loud。list 版面也可改放 `workflow.json`(#323),`load_run_callable` 以 `build_run(parse_def())` 跑它,run.py 與 JSON 並存時 **JSON 勝出**(Q6)。 |
| `src/workspace_app/workflow/manifest.py` | `WorkflowManifest` + `WorkflowPhase` — workflow 的宣告半邊(來自 `_profile.json`):穩定 `id`、相位骨架、`input_json` 位置、`wf.config`、launcher card metadata。編排本身是碼(`run.py`)或 DSL(`build_manifest` 從 `workflow.json` 衍生),不是這份資料。 |
| `src/workspace_app/workflow/dsl.py` | #323 「降級 DSL」。`WorkflowDef` = 解析後的 `workflow.json`(msgspec tagged union,tag 欄位 `type`),5 種 step:`AgentStep`/`SandboxStep`/`GateStep`/`CapabilityStep`/`MapStep`。`parse_def` decode(壞 JSON/未知欄位/壞 tag → `DslError`)、`build_run` → `ProfileRun`(trusted interpreter,把每個 step 派到 run.py 用的同一批原語:`agent_step`/`agent_write_step`/`sandbox_node`/`human_gate`/`wf.ingest_to_collection`/`wf.upsert_context_card`/`wf.map`)、`build_manifest` → `WorkflowManifest`。`_resolve` 做 deterministic、no-eval 的 `{x}`/`{x.field}` 內插;`validate_def` 回傳 static 問題清單。`CAPABILITIES`/`_CHECKS` 是 DSL 唯一可叫的能力/check。 |
| `src/workspace_app/workflow/workspace_store.py` | 使用者自編 workflow 的 on-disk 形狀(#323 P4):`WORKSPACE_WORKFLOW_DIR='.workflows'`、`slugify_workflow_id`(檔名即權威 id)、`validate_workflow_json`、`save_workspace_workflow`(canonical msgspec JSON,id 強制成 slug)、`load_workspace_workflow` → `(WorkflowDef, WorkflowManifest)\|None`(orchestrator + route 共用的單一讀取)、`workspace_workflow_metas`(panel 用,壞檔跳過)。存在 `<workspace>/.workflows/<id>.json`,與 run journal `/.workflow/<id>/` 不同。 |
...
| `DriveTurn`/`RunSandbox`/`IngestCapability`/`ConvertCapability`/`CollectionChecker`/`UpsertCardCapability`/`FindCardCapability` | `handle.py` 頂部別名 | driver 接上真實實作;測試注入 fake |
| Orchestrator 注入操作(`load_run`/`load_manifest`/`load_workspace`/`publish`/`release`/`notify_failure`/`load_upload_dir`) | `orchestrator.py` `WorkflowOrchestrator` dataclass 欄位 | API composition root(正式)/ 測試 fake |
...
## 使用者自編 workflow（DSL, #323）

除了 trusted Python `run.py`,一個 list-form workflow 也可以是一份宣告式的 `workflow.json` — #323 的「降級 DSL」。JSON 是**資料**,`build_run` 是 trusted interpreter,所以 API 裡**不跑使用者程式碼**(像 user skill 一樣讓非工程師與 AI 共同產出可跑的 workflow)。§9 的 journal + input-hash skip 原封不動:DSL 固定、內插 deterministic,每個 step 的身分(`name`/`key`)跨重跑穩定。

**Schema(`dsl.py`)。** `parse_def` 把 bytes/str decode 成 `WorkflowDef`(`forbid_unknown_fields`;壞 JSON/未知欄位/壞 step tag → `DslError`)。`steps` 是 5 種 step 的 msgspec tagged union(tag 欄位 `type`),每種派到 `run.py` 用的同一批原語:

| step | 對應原語 | 重點 |
| --- | --- | --- |
| `agent` | `agent_step` / `agent_write_step` | 設 `out` ⇒ 模型產出檔案內容並寫檔(gate 在該檔);無 `out` ⇒ 需顯式 `check`。`tools` 受 profile ceiling 限。 |
| `sandbox` | `sandbox_node` | 沙箱內 deterministic 指令,無 LLM;使用者 DSL 下 **compute-only**(無 credential,Q4)。 |
| `gate` | `human_gate` | 只在 `approve` 續行,其他終端選項 → run 結束為 `{"status": choice}`(v1 無 revise-loop)。必須是 top-level。 |
| `capability` | `wf.ingest_to_collection` / `wf.upsert_context_card` | 可靠、idempotent 的 side-effect,以 captured user authz 跑 — `CAPABILITIES` 是使用者 DSL 唯一的 side-effect 路徑。 |
| `map` | `wf.map` | 對 `over` glob 比中的每個 path fan-out 跑 `do`;**一層**(Q7:`do` 內不得再有 `map`/`gate`,`validate_def` 強制)。 |

**內插(`_resolve`)。** deterministic、async、**no-eval** 的 `{x}` / `{x.field}`,對 namespace `{config, inputs}` + 當前 map 變數解析:整個 template 剛好是一個 `{expr}` 時回傳解析值(可為 list/dict),否則做字串替換。`{x.field}` 若 `x` 是 `.json` 檔路徑,會先讀+解析(§8 decision→data→action 路由)。沒有任意運算式。

**Static 驗證(`validate_def` → `list[str]`,空 ⇒ 合法)。** 抓:schema version 非 1、缺 `id`、未宣告的 `phase`、無 `out` 的 agent 缺 `check`、`map` 內巢狀 `map`/`gate`、未知 capability/check、scope 外的 `{var}`、agent `tools` 超出 profile ceiling。`save_workflow` agent 工具把這些丟回給 AI 修;package 發現則 `discovery.validate_workflow_profiles` 啟動時對壞 DSL fail loud(`load_run_callable` 走 `parse_def`),`authoring._check_dsl` 在 `workflow check` 做完整 `parse_def + validate_def` 靜態驗證。

**儲存與遮蔽(Q5/Q6)。** 使用者版存在 workspace 的 `<ws>/.workflows/<id>.json`(`workspace_store.py`,檔名即權威 id)。orchestrator 注入 `load_workspace`,`_resolve_run`/`_resolve_manifest` 在 **start 與每次 resume** 都先試 workspace def — 遮蔽同 id 的 package workflow(Q5),沒有才退回 `load_run`/`load_manifest`(`load_workspace=None` ⇒ package-only,既有呼叫/測試不變)。dsl-aware 的 `_load_workspace` closure 在 `api/app.py` `create_app` 以 `build_run` + `load_workspace_workflow` 接上。package tier 的 `profiles/<p>/workflows/<id>/workflow.json` 是 dev promote target:`discovery.load_run_callable` 用 `build_run(parse_def())` 跑它,run.py 與 JSON 並存時 **JSON 勝出**(Q6)。route `GET /a/{slug}/items/{item_id}/workflows`(`list_item_workflows`)列出本 item 的 `.workflows/` manifest;Run guard `_workflow_manifest_or_404` 加了 workspace fallback,純 workspace 的 `workflow_id` 不再 422。
...
- **[知識庫:攝取與索引](kb-ingest-index.md)**：`ingest_to_collection` / `upsert_context_card` / `find_overwrite_card` 能力(`handle.py`)以 captured user 身分驅動 KB Ingestor 與 ContextCard CRUD(#111/#205,topic-hub →collections workflow)。`convert`(#324)在 filing 前先把上傳轉成文字:`WorkflowExecutor.convert`(`api/workflow_exec.py`)把 `convert_upload` 綁到 run workspace,跑 `Ingestor.convert`(沿用同一套 KB parser/VLM、只 parse、不 chunk/embed、無 SourceDoc),只存轉好的 markdown/text、不存原始 binary;此 node 以 src 路徑為 key journaled,(VLM)轉換不會重跑。
...
- `src/workspace_app/workflow/orchestrator.py` — `start`、`_prune_runs`、`_track`、`_execute`、`_post_run`、`cancel`、`decide`、`steer`/`_propose`/`confirm_steer`、`_on_event`/`_apply_step_record`、`load_workspace`/`_resolve_run`/`_resolve_manifest`(#323)。
- `src/workspace_app/workflow/handle.py` — `WorkflowHandle`、`journal_dir`、`ingest_to_collection`、`convert`(#324)、`upsert_context_card`、`map`。
- `src/workspace_app/workflow/dsl.py` — `WorkflowDef`/`Step`(tagged union)、`parse_def`、`build_run`(trusted interpreter)、`_resolve`(deterministic 內插)、`validate_def`、`CAPABILITIES`(#323)。
- `src/workspace_app/workflow/workspace_store.py` — `WORKSPACE_WORKFLOW_DIR`、`load_workspace_workflow`/`save_workspace_workflow`/`workspace_workflow_metas`/`slugify_workflow_id`(#323 P4)。
