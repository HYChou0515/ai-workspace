...
**Per-item skill tri-state 覆寫 + apply-this-turn(#380,對齊 #322)**:`app.json` `agent.skills` 是 skill **ceiling**;profile `_profile.json` `skills`(⊆ ceiling,`UNSET`→繼承全部)決定 shared skill 的 `default_on`。item 的 `attached_skill_prefs`(sparse `dict[str, bool]`)逐 skill pin follow/on/off——關閉的 skill 不進 prompt 的「## Available skills」index,`read_skill` 也拒絕它(除非這回合被 apply)。picker state 由 `GET /a/{slug}/items/{item_id}/skills` 提供(`api/file_routes.py`):每列 `_ItemSkillState{name,description,source(shared/profile/workspace),default_on,pref(follow/on/off),effective}`,全部走 `effective_item_skills` 這**同一個 resolver**(與回合 index 同源,anti-drift)。**Apply** 是另一條軸:composer 的 `apply_skills`(一次性、送出後自動清)把被選 skill 的整段 body 硬預載進該回合 content(小型本地模型可能略過 `read_skill` 這種工具呼叫,所以用硬預載而非軟指示),且 apply **蓋過** disable(關掉的 skill 仍可被 apply)。只作用於 App item chat;KB chat 不帶。
