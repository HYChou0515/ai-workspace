...
| `src/workspace_app/apps/base.py` | `WorkItemBase`——每個 App 的 item Struct 都繼承它。Tier1 `title`/`owner`(+ `profile`/`attached_preset`/`attached_tool_prefs`——#322 per-item tool tri-state 覆寫,`dict[str, bool]`,`attached_preset` 的姊妹欄位——加 `attached_skill_prefs`——#380 同形的 per-item **skill** tri-state 覆寫,`dict[str, bool]`),Tier2 `members`/`topics`(`T \| UnsetType`,redeclaring 才 opt-in),Tier3 = App 自己的 domain 欄位。也定義 `IndexedFields` 型別別名。 |
...
| `src/workspace_app/apps/catalog.py` | `AppCatalog`——每回合三層 resolve。`resolve(*, …, tool_prefs=None, skill_prefs=None)` → `AgentConfig`:tools/presets 是 App ceiling 的 profile 子集(否則整個 ceiling),`_apply_tool_prefs` 再把 per-item `tool_prefs` tri-state 覆寫(#322)疊上 default set 並以 ceiling 順序輸出,preset 給 model/creds/sandbox_image/idle/env,prompt = base ◇ #241 preamble ◇ profile appendix ◇ skill index——**skill index 也走 `effective_item_skills(…, skill_prefs)` 濾掉關閉的 skill(#380)**。`validate_function_coherence` + `validate_all_apps` 在啟動時 enforce decision 11。 |
...
| `src/workspace_app/apps/profiles.py` | `ProfileManifest`(`_profile.json`)——narrows `tools` ⊆ app.tools / `skills` ⊆ app.skills(#380;`UNSET`→繼承全部)/ `presets` ⊆ app.picker,加 `suggestions` / `default_preset` / `collections`(#280)/ `upload_dir`(#198)/ `workflows`(#100)。`load_profile` / `load_profile_appendix` / `list_profiles` + workflow 正規化 helper。 |
...
| `src/workspace_app/apps/skills.py` | 漸進揭露 skills。Package skill 在 `profiles/<p>/.skill/<name>/SKILL.md`(frontmatter `name`/`description` + body,`read_skill` 按需載入)。`list_skills`/`load_skill`(`@cache`)、workspace skills(#298,即時讀不快取)、`merged_profile_skills`,加上 `save_skill` 工具(在 `agent/tools.py`)用的 helper `slugify_skill_name`/`render_skill_md`。**#380**:`SkillState{name,description,source,default_on,effective}` + **單一真相** resolver `effective_item_skills(app_slug, profile, prefs, workspace_metas)`(去重優先序 workspace>profile>shared;shared `default_on` = `name in profile.skills-or-all`)——picker 端點與 prompt 的「## Available skills」index 都走它(anti-drift);`build_workspace_skills_block(…, prefs)` 用它濾掉 `pref is False` 的 skill;`resolve_skill_body`/`build_applied_skills_block` 供 apply 這回合硬預載 skill body。 |
...
`api/locator.py:ItemLocator.resolve_agent_config(item_id)`(post-#54;原 `app.py` 的 `_resolve_agent_config` closure 改名搬進 `ItemLocator`,去掉前綴底線)委派給 `resolve_item_agent_config(spec, app_catalog, item_id)`:`find_work_item` 掃 `registered_apps()` 得到 `(slug, item)`,再 `app_catalog.resolve(app_slug=slug, profile=item.profile, attached_preset=item.attached_preset or None, tool_prefs=item.attached_tool_prefs or None, skill_prefs=item.attached_skill_prefs or None)`(keyword-only 簽名)合成 tools(profile ⊆ app ceiling)、allowed presets(profile ⊆ app.picker)、chosen preset(attached → profile default → 第一個),以及 system prompt(base `prompt_file` ◇ #241 workspace preamble(僅 workspace App)◇ profile `_prompt.md` appendix ◇ merged skill index——關閉的 skill(`skill_prefs` pin False)不進 index,#380)。同一回合,`api/chat_send.py:ChatSendService.send`(post-#54;原 `app.py` 的 `_send_into` closure)也呼叫 `build_context_block` 把 App `agent.context_files` 的即時內容 prepend 到 agent content(永不存進 history),另外 `build_workspace_skills_block(…, prefs)` 把使用者在此 workspace 共創的 skill 索引一併 prepend(同樣濾掉關閉的);若這回合帶了 `apply_skills`,`build_applied_skills_block` 再把被選 skill 的**整段 body** 硬預載進 content(一次性,不進 history)。

**Per-item tool tri-state 覆寫(#322)**:`app.json` `agent.tools` 是 **ceiling**——可勾選工具的全集,也是任何覆寫的硬上限;profile `_profile.json` `tools` 只是 default-checked 子集(省略則整個 ceiling)。item 的 `attached_tool_prefs`(sparse `dict[str, bool]`)逐工具 pin:`True` 強制開、`False` 強制關、不在 dict 的鍵則 follow default。覆寫 ceiling 是 **app.json 而非 profile**,所以 force-on 能把 profile 收窄掉的工具加回來。`catalog.py:_apply_tool_prefs` 有 pref 時以 ceiling 順序輸出、沒有 pref 時原樣回 profile 的順序(deterministic)。**指令粒度(plan-tools-picker-groups 第二部分)**:resolve 只能在條目層級 pin(它跑在回合解出第三方套件之前),所以 `AgentConfig` 帶著 `tool_ceiling`／`tool_prefs` 到「packages 齊了」的門——`turn_context._finalized`(每種 turn)、WUI `callTool`、picker route、replay loader——在那裡 `apps/catalog.py:finalize_tool_grants` 用 `tooling/catalog.py:command_grants` 把 `allowed_tools`／`disabled_tools` **本身**寫成指令粒度(整包授權 → 該套件每個 `pkg:cmd`,逐指令套 pin;`unit_pref`:指令鍵 > 整包鍵 > 預設)並把兩個欄位用掉清空,之後每個分得出「套件指令」和「套件」的讀者(runner、provisioning、sub-agent clamp、`_wui_callable`;sizing 與 authz 只看內建名,看不出差別)拿到的就是最終答案;收窄(workflow step 的 `tools:`、sub-agent 定義的 `tools`)在定案**之後**用 `narrow_entries` 做交集,所以 pin 不會再把收窄放寬。picker state 由 `GET /a/{slug}/items/{item_id}/tools` 提供(`api/tools_routes.py:register_tools_routes`):每個 `picker_units(expand_entries(ceiling, packages), packages)` 一列 `ItemToolState{key,label,description,default_on,pref(follow/on/off),effective,group,package?}`,其中 `effective` 取自 `finalize_tool_grants(locator.resolve_agent_config(item_id), packages)` 的 `allowed_tools`——**與真正回合走的同一條 resolve、同一個定案函式**(anti-drift),`default_on` 取自 `profile_default_tools`(resolve 自己起手的那段;`tools: []` 是零不是繼承);`GET /tools` 則回傳 flat catalog 給 chat tool cards 標 label(用 backend catalog 取代 FE raw-name fallback)。

**Per-item skill tri-state 覆寫 + apply-this-turn(#380,對齊 #322)**:`app.json` `agent.skills` 是 skill **ceiling**;profile `_profile.json` `skills`(⊆ ceiling,`UNSET`→繼承全部)決定 shared skill 的 `default_on`。item 的 `attached_skill_prefs`(sparse `dict[str, bool]`)逐 skill pin follow/on/off——關閉的 skill 不進 prompt 的「## Available skills」index,`read_skill` 也拒絕它(除非這回合被 apply);**這個 item 完全沒東西可載時,連 `read_skill` 這個工具都不會被建**——`turn_context._skills_reachable` 問兩個產生器:App 那半問這同一個 resolver,workspace 那半問 `advertised_workspace_skills`(block 自己的規則,只藏明確關掉的)。兩者在「default-off package skill 的 workspace 副本」上**故意不一致**:block 會列、`read_skill` 也載得到,所以那種情況算「載得到」。答案經 `ctx.skills_reachable` 交給 runner。picker state 由 `GET /a/{slug}/items/{item_id}/skills` 提供(`api/file_routes.py`):每列 `_ItemSkillState{name,description,source(shared/profile/workspace),default_on,pref(follow/on/off),effective}`,全部走 `effective_item_skills` 這**同一個 resolver**(與回合 index 同源,anti-drift)。**Apply** 是另一條軸:composer 的 `apply_skills`(一次性、送出後自動清)把被選 skill 的整段 body 硬預載進該回合 content(小型本地模型可能略過 `read_skill` 這種工具呼叫,所以用硬預載而非軟指示),且 apply **蓋過** disable(關掉的 skill 仍可被 apply)。只作用於 App item chat;KB chat 不帶。
