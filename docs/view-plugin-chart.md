...
- 在哪張圖選取，那張圖也照 marking 點亮（框留著，可以看、可以清），和其他 view 一致；只有不寫 marking 的圖
  ...
  也不會消失。（grid 的格子與圓餅圖點的一片例外：自己選到的亮、其餘以透明度變暗，取代 marking 的點亮。）
...
- 圖上的「N selected · by <欄位>」數的是寫進 marking 的列：框到疊圖的一段、而那一段不寫東西時，不算進去；key 欄位
  沒有值的列也不算，框到的列在 key 欄位都沒有值時不寫 marking（不會清掉其他 view）。計數排在最前面、不會縮小讓位給說明列；
  只有當一行放不下計數加上每則說明的「…」時，計數才以「…」收尾；說明列放不下時以「…」收尾、至少留下「…」，不會整則消失；完整文字在滑鼠停留的提示裡。`keys:` 的欄位整欄都沒有值時，`show_file` 以「keys:
...
- 對數軸上 0 與負值沒有位置，那些點不畫，圖上方的說明列寫「N values at or below 0 not drawn on the log y axis」，
  ...
  畫成一個點；水平的折線也是。疊圖為了對齊而補的 0 不是那一層自己的值，不畫成點。疊圖先依位置與顏色加總，
  負值會算進那一段的總和（不另外計數）；說明列數的是加總之後仍沒有位置的段。
