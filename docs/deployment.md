...
- **API pod 收到 SIGTERM 怎麼收攤（plan-graceful-shutdown）**：rollout、HPA 縮容、
  ...
  4. lifespan 的 turn drain：在跑的 turn 有同一個 budget 跑完並存檔（含 budget 內才輪到
     ...
     **不寫**「interrupted」標記（那是按 Stop 的意思），由別的 pod 乾淨重跑；**KB 聊天**
     沒有認領（見下一條），在跑的超過預算照舊 cancel、存 partial、寫標記；排隊中的留在 queue 裡
     沒人跑（除非 teardown 期間同一把 key 又有 enqueue 把 worker 重生，那會先把舊的跑掉——app
     聊天的認領已放手所以什麼都不寫、只是白跑；KB 的跑完會存）；還在 route 裡組 turn 的那一段不受
     drain 管，會在 drain 之後才排進去、由重生的 worker 跑、跑完前 process 死了就沒了（問題留在
     對話上，跟 #815 之前一樣）。`run_consumers: true`
...
- **pod 死了，正在回的 turn 怎麼辦（plan-graceful-shutdown P3/P4，只涵蓋 app 聊天）**：
  ...
  **已知限制**：(1) 使用者在某則訊息還在**準備**時按 Stop、pod 又在它輪到前 drain 掉，
  ...
  tick **都先列表、再各自 take**，又恰好在心跳過期的兩側各讀一次，候選名單不同，同一把 key 仍可能
  被分著拿（一邊的重跑被當「interrupted」砍掉）——還要未 released 的那張排在 released 的前面；
  `taken_at_ms` 擋的是「列表落在對方 take 之後」的那種，擋不了兩邊都先列好的；(4) 交接時的
  release 跑在 thread pool 上，pool 被 VLM 串流佔滿到 2 秒內排不到就放棄交接，退回 P4 之前的
  行為（log 一行 ERROR 含 traceback；partial 存檔——除非晚到的 release 趕在副本問 `is_mine` 之前
  落地，那副本就什麼都不寫、由 peer 重跑）。
