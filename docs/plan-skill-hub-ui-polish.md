...
**狀態：** P1–P11 建好，review 三輪（四把鏡頭 → 三把 → 兩把）的發現都修了，第三輪只剩文字（2026-09-19，PR #826 draft，等 CI 與你的點頭）。來源是 #818 合併後錄的九支 /web-demo、你看片時提的六條、以及 7 個介面 × 6 種視窗的小尺寸量測。以前端為主；後端的兩條（工具回話的換行、給人看的伺服器句子）**也一起**——它們不是機制問題，一條是分段、一條是照抄站內既有的「code + 參數、前端翻譯」形狀。#822（技能面板一列的 cluster 換行）**併進來**，那條 draft PR 在本 PR 合併時關掉。
...
| **P3** | D3 silentError（picker + 條目頁五個 mutation：下架／重新上架共用一個）；D10 `PageNotice` + 轉移／刪除後的成功提示 | 測試：被拒的 install 只出現一處錯誤文字、`reportWriteFailure` 未被呼叫（spy）；轉移成功 → 列表頁 `role="status"` 含新 owner 顯示名；刪除同 |
...
- **D1 是整站的視覺改動**：預設 20 會讓沒傳 padding 的對話框多出留白（數過：9 個沒傳，2 個要這個留白，7 個自己排版——正是要確認的那一類），但也要確認沒有呼叫者是「刻意沒傳、靠子元素自己留白」——P1 逐個截圖看。
...
## Review 第二輪與第三輪（2026-09-19）

第二輪（對 `896e9e33`，回歸＋真實性＋只看修法的缺陷鏡頭）：換掉的機制果然有貨——「…」的 popover 畫在 nav 的 `overflow: hidden` 裡面，真瀏覽器裡開了等於沒開（第一輪的「量到在畫面上」量的是 bounding rect，看不到裁切）→ 裁切搬到包住麵包屑的內框（P17）；守衛讀的是字樣不是 key（單行物件、註解裡的 `padding:`、tag 內註解的撇號）→ 先抹掉註解再用大括號配對讀（P18）；D3 的兩個對話框把錯誤畫在背後的頁面上 → 對話框自己畫（P18）；列表頁首次載入失敗後再搜尋又整棵樹變載入中、空 hub 的 refetch 失敗被空狀態蓋住（P18）；`copy_of` 漏了一條整列比對的既有測試（P18）；三個錯句（P19）。

第三輪（對 `006257c6`，只看 P17–P19 換掉的機制）：沒有回歸；三格還是錯的——失敗在對話框「打開」時清掉（該在下一個動作「開始」時清，且對話框只畫自己那個動作的失敗）、失敗後按 Retry 又整棵樹變載入中（「從沒有結果也沒有錯誤」才算第一次載入）、JSX 文字裡的撇號讓守衛的註解抹除失效（引號到行尾為止）→ P20；其餘是文字（P21）。

階段編號：表裡的 P12 是「review → CI → demo」這一步；commit 用 P12–P21 記錄各輪修法，是同一步的細目。
