...
- **`pip` 就是 carrier 的 pip。** `python`/`python3*` **與** `pip`/`pip3*` 都 shim 到 carrier
  `launch`(它依被叫的名字分派,`pip` → `python -m pip`),所以 `pip install X` 裝進的正是
  `exec(["python", ...])` 會跑的直譯器。bundle 直譯器的 PEP 668 `EXTERNALLY-MANAGED` 標記在
  prebuild 時移除——它是 uv 發行版帶進來的,對一個沒有 packager 追蹤、也沒有 venv 可退的自帶
  bundle 不成立;移除後 pip 自己退到 `--user`,正好落在上面那個 `SANDBOX_HOME`。
- **login shell 的 PATH guard。** `bash -lc` / workflow 的 `sh -lc` 會 source `/etc/profile`,
  Debian 的 profile 硬重設 PATH 把 per-sandbox `.jailbin` 丟掉。jail 用 tmpfs 蓋
  `/etc/profile.d`;沒有 jail 時(線上就是這個)由映像檔安裝
  `docker/profile.d/sandbox-jailbin.sh`,從 per-exec 匯出的 `SANDBOX_JAILBIN` 把目錄補回 PATH 最前面。
