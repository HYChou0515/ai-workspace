...
| 2026-09-18 | **graceful shutdown + turn 換 pod 接手（plan-graceful-shutdown，PR #815）**：`server.shutdown_budget_sec`（預設 20，也是 uvicorn `timeout_graceful_shutdown`）、`server.turn_reclaim_interval_sec`（預設 5，0 = 關）、新 model `turn-claim`（post-apply 註冊、無 CRUD 路由） | 以前 uvicorn 在 SIGTERM 後**無限等**所有連線結束才跑 lifespan shutdown，SSE 永不結束，所以只要收到信號時有人開著聊天，turn drain / sandbox 拆除就跑不到、pod 30 秒後被 SIGKILL（本機探針 + uvicorn 原始碼；線上 log 沒看過）。現在 SIGTERM 當下 readyz 503、所有 SSE 送 EOF（前端重連到活的 pod）、lifespan 在**一個 deadline** 內 drain：跑完的存檔；**app 聊天**跑不完的先放掉認領再 cancel、**不寫** partial / 「interrupted」；**KB 聊天沒有認領**，照舊 cancel + partial + 標記。另外 `read_image` / `read_page` / 畫圖審圖 / doc-question 回答 的同步 LLM 呼叫從 event loop 搬到 thread（P1）——這是 liveness probe 被殺的元凶。**turn 換 pod 接手（P3/P4，app 聊天限定）**：每次 `ChatSendService` 的 send 在存下問題的同時開一列耐久認領（帶完整 send 配方，**不含**請求組出來的 env；重跑問 `env_without_request`），回覆存檔時刪掉；fleet 裡一顆 pod（`ScanLease`）每 5 秒掃、以對話為單位：released 立刻接；心跳過期就把該對話的認領全接下、推一次 `TurnEpoch`、照順序重跑；重跑上限 2 次，超過寫 error 結尾。shutdown 的 sandbox 拆除改用 `kill_idle` 的規則（fleet 閒置才 kill，否則 write-back + 丟本地 session）。**部署後不用跑任何 migrate**；升版當下在跑的 turn 沒有認領列，那一批仍是舊行為。`run_consumers: false` 不影響（sweeper 產出的是 turn，只有 API 能跑）。**k8s 側**：`terminationGracePeriodSeconds: 90`（≥ 2 × budget + 8 + preStop 5 + 拆除）、`preStop: sleep 5`。 |
