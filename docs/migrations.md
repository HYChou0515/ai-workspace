...
| `server.shutdown_budget_sec`（新，預設 20）+ `rca-app` 的 `terminationGracePeriodSeconds: 60` / `preStop sleep 5` | plan-graceful-shutdown（2026-09-18） | ⚠️ **行為有變，而且是「rollout / HPA 縮容時 chat 報廢」的修法**：以前 SIGTERM 來了 uvicorn 會**無限等**所有連線結束才跑 lifespan shutdown，而每個開著的聊天分頁都掛著一條永不結束的 SSE，所以 turn drain（#558）和 sandbox 拆除在 prod **從來沒跑過**，每次都是 30 秒後 SIGKILL（本機探針證實：`Waiting for connections to close.` 之後永遠不動）。現在 SIGTERM 當下就開始 drain：`/api/readyz` 立刻 503、所有 SSE 送 EOF（FE 視為斷線、自動重連到活著的 pod）、uvicorn 的等待以 `shutdown_budget_sec` 為上限、lifespan 的 turn drain 也用同一個數字（超過就 cancel，partial 回覆會存，跟按 Stop 一樣）。**不設什麼都不用改**；k8s 側（你自維護）要把 `rca-app` 的 `terminationGracePeriodSeconds` 拉到 > 2×budget（base 給 60）並加 `preStop sleep 5`；`run_consumers: true` 的單機部署，job queue 的 drain 現在也受同一預算約束（本機開機的 help 文件 index 就要 16 秒），沒排完的 job 交給 specstar 的 stale-job recovery。另外 `read_image` / `read_page` / 畫圖審圖的 VLM 呼叫從 event loop 搬到 thread（P1）——這是 liveness probe 被殺的元凶 | `docs/deployment.md`；`kubernetes/base/deployment.yaml` |
