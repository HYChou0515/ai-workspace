...
| （無新選項）blob GC 改成 job：`blob-gc` JobType + `rca-worker-blob-gc` | plan-blob-gc-job（2026-09-17） | ⚠️ **行為有變,而且是 API pod OOM 的修法**：孤兒 blob 的 reconcile（specstar `gc(mode="reconcile")`）**不再在 API pod 上執行**。以前搶到 CAS lease 的那顆 API pod 每 `gc_interval_sec` 把每一個 blob-capable model 的**所有 revision 一次載進記憶體**算 live 集合——pod 死前最後一行就是 `blob-gc: won lease`。現在 API 每個 window 只 **enqueue** 一個 `BlobGcJob`（同一份 window ledger 上的 `__scan__:blob-gc` lease,fleet-wide 一個 window 一顆 pod 問;已有 queued/running 的 coalesce;每趟開始前先硬刪其他終態列,row 不隨 uptime 長）,真正的工作在 **`rca-worker-blob-gc`** 上跑。**這個 worker 開機組的是 API 自己那整套**（`__main__.build_app`,只組不 serve）——reconcile 只從已註冊的 model 算 live 集合,有 `dict[str, Any]` 欄位的 model 也會被掃,拿部分註冊表跑會把缺的 model 引用的 blob 隔離、`gc_t2` 後刪掉;所以 registry 由建構保證相同,而且每次 enqueue 都帶 API 端的 model 清單,runner 少任何一個就拒跑（job FAILED、log 點名）。**不設什麼都不用改**;`server.run_consumers: false` 而沒有跑 `rca-worker-blob-gc` 的部署,孤兒 blob 就不再回收（只長不消,不掉資料;以前是 API 做掉,也是它 OOM 的原因）。舊的 `_GcLease` model 與它那一列 `blob-gc` 不再被讀,留著無害;程式碼已刪 | **怎麼起 worker:deployment.md §11「Job runner ⊥ API」**;`kubernetes/base/workers.yaml`(`rca-worker-blob-gc`,記憶體要照「一次載入所有 revision」給) |
