...
| `server.shutdown_budget_sec`（新，預設 20）+ `rca-app` 的 `terminationGracePeriodSeconds: 60` / `preStop sleep 5` | plan-graceful-shutdown（2026-09-18） | ⚠️ **行為有變，而且是「rollout / HPA 縮容時 chat 報廢」的修法**：以前 SIGTERM 來了 uvicorn 會**無限等**所有連線結束才跑 lifespan shutdown，而每個開著的聊天分頁都掛著一條永不結束的 SSE，所以 turn drain（#558）和 sandbox 拆除在 prod **從來沒跑過**，每次都是 30 秒後 SIGKILL（本機探針證實：`Waiting for connections to close.` 之後永遠不動）。現在 SIGTERM 當下就開始 drain：`/api/readyz` 立刻 503、所有 SSE 送 EOF（FE 視為斷線、自動重連到活著的 pod）、uvicorn 的等待以 `shutdown_budget_sec` 為上限、lifespan 的 turn drain 也用同一個數字（超過就 cancel，partial 回覆會存，跟按 Stop 一樣）。**不設什麼都不用改**；k8s 側（你自維護）要把 `rca-app` 的 `terminationGracePeriodSeconds` 拉到 > 2×budget（base 給 60）並加 `preStop sleep 5`；`run_consumers: true` 的單機部署，job queue 的 drain 現在也受同一預算約束（本機開機的 help 文件 index 就要 16 秒），沒排完的 job 交給 specstar 的 stale-job recovery。另外 `read_image` / `read_page` / 畫圖審圖的 VLM 呼叫從 event loop 搬到 thread（P1）——這是 liveness probe 被殺的元凶。**turn 換 pod 接手（P3/P4，新 model `turn-claim`、新旋鈕 `server.turn_reclaim_interval_sec` 預設 5）**：每一次 send 在存下問題的同時開一列耐久認領（帶完整的 send 配方），回覆存檔時刪掉；每顆 pod 每 5 秒（`ScanLease`，fleet-wide 一窗一 pod）掃「心跳過期且對話還停在那則問題」或「原 pod 放手」的認領，CAS 搶下、推進 `TurnEpoch`（卡住沒死的原 pod 會自己取消副本）、在自己的 engine 重跑；SIGTERM 時預算內跑不完的 turn 先放手再 cancel、**不寫** partial/「interrupted」標記，由別的 pod（或重啟後的自己）產出乾淨的 Q→A。**部署後不用跑任何 migrate**（新 model、post-apply 註冊、無 CRUD 路由）；升版當下在跑的 turn 沒有認領列，那一批仍是舊行為（pod 死就斷）。`run_consumers: false` 不影響——這個 sweeper 產出的是 turn，只有 API pod 能跑，所以留在 API | `docs/deployment.md`；`kubernetes/base/deployment.yaml` |
