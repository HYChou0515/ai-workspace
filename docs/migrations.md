...
| `server.shutdown_budget_sec`（預設 20）、`server.turn_reclaim_interval_sec`（預設 5，0 = 關）；新 model `turn-claim`（post-apply 註冊、無 CRUD 路由） | plan-graceful-shutdown（2026-09-18，PR #815） | **不設也會生效（都有預設）**。以前 uvicorn 在 SIGTERM 後**無限等**所有連線結束才跑 lifespan shutdown，SSE 永不結束，所以只要收到信號時有人開著聊天，turn drain / sandbox 拆除就跑不到、pod 30 秒後被 SIGKILL（本機探針 + uvicorn 原始碼；線上 log 沒看過）。現在 SIGTERM 當下 readyz 503、所有 SSE 送 EOF（前端重連到活的 pod）、lifespan 在**一個 deadline** 內 drain：跑完的存檔；**app 聊天**跑不完的（在跑、排隊、還在準備的都算）先放掉認領再 cancel、**不寫** partial / 「interrupted」；**KB 聊天沒有認領**，照舊 cancel + partial + 標記。另外 `read_image` / `read_page` / 畫圖審圖 / doc-question 回答 的同步 LLM 呼叫從 event loop 搬到 thread（P1）——這是 liveness probe 被殺的元凶。**turn 換 pod 接手（P3/P4，app 聊天限定）**：每次 `ChatSendService` 的 send 在存下問題的同時開一列耐久認領（帶完整 send 配方，**不含**請求組出來的 env；重跑問 `env_without_request`），回覆存檔時刪掉；fleet 裡一顆 pod（`ScanLease`）每 5 秒掃、以對話為單位、整把 key 一起拿或一起不拿：released 立刻接；心跳過期就把該對話的認領全接下、推一次 `TurnEpoch`、照順序重跑；重跑上限 2 次，超過寫 error 結尾（使用者看得到、可以重送）。認領就是帳本：回覆存進去才刪，不看對話尾巴猜——代價是 pod 恰好死在「回覆已存、認領還沒刪」那一毫秒時會多一個答案。shutdown 的 sandbox 拆除改用 `kill_idle` 的規則（fleet 閒置才 kill，否則 write-back + 丟本地 session）——`kind: local` 單機重啟後最近用過的 item dir 會留在 scratch 上（跟以前 crash 一樣），碰到才被 idle reap。**部署後不用跑任何 migrate**；升版當下在跑的 turn 沒有認領列，那一批仍是舊行為。`run_consumers: false` 不影響（sweeper 產出的是 turn，只有 API 能跑）。**k8s 側**：`terminationGracePeriodSeconds: 90`（≥ 2 × budget + 8 + preStop 5 + 拆除）、`preStop: sleep 5`。 | [docs/deployment.md](deployment.md) §「API pod 收到 SIGTERM 怎麼收攤」與下一條；`kubernetes/base/deployment.yaml`；`plan-graceful-shutdown.md` |
