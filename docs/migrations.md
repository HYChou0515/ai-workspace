...
- 隨之而來、沒有開關的三個語意（都是「picker 關掉的指令就是關掉」）：workflow step 的 `tools:` 寫 `rca-tools` 時拿到的是
  **這個 item 持有的那幾個指令**（被釘掉的不在）；sub-agent 定義的 `tools` 可以寫整包名（`save_subagent` 接受、載入時縮成持有的指令）；
  WUI 頁面叫一個被釘掉的指令會 403。一個整包鍵現在也管到 app 只授部分指令的套件（以前那種鍵是 no-op）。
