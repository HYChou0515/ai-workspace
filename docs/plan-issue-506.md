...
- **P5**(**DONE**,commit `32a56f1e`)**生成升級**:`AgentCardDrafter`(`api/card_drafter_agent.py`)用配好的 `ask_knowledge_base`(scope=[cid] / budgets / prompt)動態查 RAG+wiki+glossary 才起草(②③)+ `card_drafting_agentic.md` + `_process_one` 接線(`digest(collection_id=ref.collection_id)` cid 貫穿、one-shot drafter 忽略之即無-cid open-loop fallback)+ `create_app` `set_drafter(AgentCardDrafter(...))` 換裝(`card_drafter_llm` 有值才建)。**Deferred(worker parity):** `build_coordinators`(API+worker 共用)只建 one-shot `LlmCardDrafter`;agentic drafter 只在 `create_app` 後 `set_drafter`。#312 split 部署(`run_consumers=false` + 獨立 card-gen worker pod)下 worker 走 one-shot(retriever+bridge+catalog worker 未建);預設 all-in-one 部署 API 內跑則走 agentic。要 worker 也閉環需把 retriever/bridge 接進 `build_bundle`。
