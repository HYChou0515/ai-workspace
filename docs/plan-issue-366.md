...
- **P7** — sweeper 韌性：registry 的 `kill_idle`/`enforce_quota`/`mirror_warm` 每個 item 包 try/except-continue；`kill` 對 `SandboxNotFound`（host 已 TTL 回收的殭屍 handle）改用 `contextlib.suppress` → 仍清掉本地 session（治本，不無限重試）；其他錯誤跳過留待下輪。測試：殭屍 + flaky item 都不會弄死 reaper/quota/mirror sweep，其餘 item 照常回收。（lifecycle backstop 不需要：registry 已不對 item 錯誤上拋，task 不會因此死。）✅
- **P8** — 收尾接線：`create_app` 在 `isinstance(sandbox, HttpSandbox)` 時注入 `SpecstarAddressStore`（lifespan `register_sandbox_address`，post-`spec.apply` 不emit CRUD 路由），local/mock 不注入、行為不變；更新 CLAUDE.md/docs。測試：http app 開機註冊 `_SandboxAddress` model、mock app 不註冊。gate：全套 + 100% coverage（docker 可用→含 integration）。#372 已關、#369 本方案繞開不需要。
