...
- **P6** — 切換 + 去特例：sync（restore→`mark_ready`、mirror→`is_ready`）、回收改用工作區外 marker；移除工作區內 marker 與所有 `.ready` walk-排除。測試：`.ready` 絕不出現在 `walk`/`stat_all`/檔案樹；三明治語意用 `is_ready` 後不變。
