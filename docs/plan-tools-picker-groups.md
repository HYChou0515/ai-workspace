...
| 2 | 預設展開／收合 | 打開時**全部收合**；「狀態混合的組」與「搜尋有命中的組」自動展開。收合狀態每次打開重來，不記。**P6 修訂（review 抓到）**：開或收只在兩個時刻決定——modal 開啟時（那時混合的組開）與搜尋詞改變時（有命中的組開、使用者親手收過的釋放）——中間只有使用者自己的點擊會動它；第一版每次 render 重算，混合組裡把最後一列按回一致，整組在游標下瞬間收起；手動收合又永久壓過搜尋命中，標題畫著、命中列不見 | user：「可以」。收合時標題列已有三態和項數，看得出整組狀態；混合才需要看進去 |
...
| P7 | 換了開合機制 → 再一輪 review（單一問題：開合／搜尋語意與它的釘子），乾淨後推、draft PR、對最終 sha 跑 CI | — |
