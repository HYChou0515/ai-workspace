...
**怎麼驅動**:搶「這一列 + 這一輪」那把鎖(CAS)→ 搶到才開 run,帶 payload、**用 item owner 的身分**(見下面訂正)→
把結果記在那一列的狀態上。
...
| 身分失效 | ~~建立者不能在此 item 行動 → **停下來並顯示原因**,任何人可接手~~ **⚠️ 已被後續決定取代,見下方「訂正:身分」** | 繼續跑=權限外洩且沒人會發現;安靜停=報表有一天不來了、幾週後才有人問 |
...
### 訂正:身分(這一段是 review 才發現文件沒跟上)

上面兩處寫「建立者的身分」,是**這一輪較早的版本**。後來拍板的是
「**頁面的自動化操作一律用 item owner 的身分**」,程式碼(`_owner_of_item`)照那個做,
而這份文件沒跟著改——所以有一段時間它同時記著兩套規則。現在只記一套:

- **排程跑的時候用 item owner 的身分。** 排程沒有請求、沒有 cookie,所以本來就沒有個人憑證
  可以繼承;而 item 已經是其他每一條規則的邊界。
- 因此 **Q6「建立者失去權限就停下來」不適用也未實作**:沒有「建立者」這個欄位,
  owner 對自己的 item 永遠有權限。它原本要防的洩漏,在 owner 模型下換成
  「**能寫進這個 item 的人,可以讓 owner 跑 profile 列出的 workflow**」——
  這條由 item 的寫入權限本身管,而 `workflows_for` 的天花板管得住跑什麼。
- ⚠️ **還沒做的那半**:沒有任何介面看得到「這個 item 上有哪些排程」(Q5),
  所以「離職的人排的每天打 DB」仍然要先知道是哪個資料夾才找得到。
