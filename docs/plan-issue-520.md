...
## 後續：#551 排除側的鏡像問題（已修，PR #554）

`exclude_doc_ids` 只比對 `source_doc_id`。在 #104 內容共用下，這條排除**兩個方向都會錯**：

- **排過頭**：共用 chunk 剛好記著被擋的擁有者 → 排掉它等於拿走唯一一份，**可讀的兄弟文件
  憑空消失**（正確性問題：你自己的文件因為別人上傳過一樣的位元組而搜不到）
- **排不乾淨**：`_DocJoin` 挑最早建立的擁有者、不看權限 → **引用標到被擋文件**，洩漏它存在

當時刻意留在本 PR 之外，因為這是**語意決定不是 bug 修**——最直覺的「排除也改成比對內容雜湊」
（對稱於本 PR 的正向過濾）會讓「排過頭」**更嚴重**。

**定案規則（#551 / PR #554）**：內容只要還有**任何一個讀得到的擁有者**就保持可搜尋，一個都
沒有才消失；引用必須指向讀得到的那一個。述詞＝`被擋 OR 內容有可讀擁有者`；`_DocJoin` 挑
canonical 時跳過被擋者。⚠️ **別再「補對稱」**——那正是會讓事情變糟的那條路。
