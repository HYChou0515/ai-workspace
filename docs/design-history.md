...
| [plan-profile-python-env.md](plan-profile-python-env.md) | profile 自帶 python 環境:起始 `pyproject.toml`+`uv.lock` 決定 sandbox 套件,每次冷啟動 `uv sync --frozen`;附「為何 venv 不放 workspace」與「共用 cache 為何必須唯讀(hardlink 別名)」 |
