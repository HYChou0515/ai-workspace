...
**狀態:** grill 十題全部問過並定案(2026-09-18),已點頭,P1–P12 施工完成、第一輪四把鏡頭 review 的修正在 P13–P17、第二輪的在 P18、第三輪(回歸 + 真實性,只審 P18)的在 P19(PR #818);施工中改掉的形狀已回寫在各段。**P11 的 `skill_eval --control` 實跑(2026-09-19,本機 ollama CPU、App 預設 preset `qwen3-local` = `ollama_chat/qwen3:14b`,`--num-ctx 16384`):四個情境有 skill 4/4 過,**沒 skill 也 4/4 過**——報告自己判「沒有證據」。驗收條「至少一個情境對照組不過」在這顆模型上**沒達成**:tool 描述本身已足以讓 qwen3 做對這四件事,所以這四個情境量不到 guidance。這是模型相關的結果(guidance 本來就是 model-specific),部署方要拿自己的模型跑;要讓情境有鑑別度得再設計會讓「沒 skill」做錯的題目,留作後續。順帶量到一個既有的工具缺陷:`--preset` 給一個不在 App picker 裡的名字會被 `AppCatalog.resolve` 靜默換成 picker 第一項,報表標頭卻印你給的名字——skill_eval 該拒絕而不是沉默(不在本 PR 修)。
