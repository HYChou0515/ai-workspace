...
> 以及**被否決的替代方案與否決理由**。
>
> ⚠️ **驗收基準線已經跑過** —— 見 **§4.5**。topic-hub 今天就具備全部前提
> （prompt 叫 agent 寫記憶、`context_files` 有注入、檔案工具已給），所以決定 2 不用等實作
> 就測得到。觀測到的失效：**14B 模型不寫記憶、改去呼叫 `save_skill`、寫入時捏造內容
> 並反轉使用者身分**。
>
> 但這些是**機率性**的，不是每次都發生（§4.5.1）—— 同一輸入重跑，模型也會正確寫入。
> 因此 Phase 2 的 DoD 改成 **N 次取樣的通過率**，而不是單次綠燈。
...
把 `num_ctx` 開到 40960、其餘一律不動（**未修改的 topic-hub prompt**、同一段輸入、
同一顆 `qwen3:14b`、app 自己組出的 24,076 字元 system prompt + 18 個工具）：

| | 未修（現況） | 修好窗口 |
|---|---|---|
| 端點實際讀入 | 4,096 | **9,940**（4/4 一致） |
| 發出合法的記憶寫入 | 未曾成功 | **1 / 4** |
| 走錯機制 | — | 3 / 4（`resolve_collection` ×2、`update_todos` ×1） |

那次成功的是 `edit_file`，`path=MEMORY.md`，**參數齊備合法**。

**兩個結論：**

1. **鏈條每一個環節都被證明可行。** ①模型能發出合法寫入 → ②③檔案落地 → ④重讀 →
   ⑤注入 → ⑥跨 chat 拿得到。沒有任何一環是壞的或未知的。
2. **截斷是必要修正，但不是充分修正。** 窗口修好後命中率只有 **1/4** ——
   §4.5 觀測到的「走錯機制」有一部分是真的，不能全部歸給截斷。
   差別在於：**修之前那是不可測的**（指示根本沒送到模型眼前），
   **修之後才有可以拿來比較的基準**。

> **Phase 2 的定位因此確定**：它要做的是把這個 1/4 拉高，而 §4.5.0 的 `num_ctx`
> 修正是它的**前置條件** —— 在截斷的端點上調 prompt，等於對著看不見的東西調參數。

#### 4.5.1 ⚠️ 失效是**機率性**的，不是確定性的

上面那一輪是**單次**觀測。用同一個 prompt、同一個模型、同一段輸入重跑，模型**會**正確地
呼叫 `edit_file` 寫進 `MEMORY.md`，兩條事實都寫對。所以正確的描述是
**「這條路徑有一定機率走錯」**，而不是「它一定走錯」。

這件事直接改寫驗收方式：

> **單次 live check 不構成通過。** 綠燈可能只是抽到好的那一次。
> 驗收必須是**同一輸入重複 N 次的通過率**，且失敗樣態要分類
> （沒寫／走錯機制／捏造／語意反轉）。

#### 4.5.2 ⚠️ 真實的 system prompt 有 ~19k 字元的工具清單

`api/litellm_runner.py` 的 `_agent_for` **不是**只送 `_compose_prompt` 的結果。它還會附加：

- `format_tools_for_prompt(tools)` —— **整份工具清單的散文版**（名稱 + 描述 + JSON args
  schema），註解寫明目的是「so small local LLMs don't confuse provisioned function tools
  with shell binaries」
- `speaker_note`（#242）與 `search_allowance_note`（#537）

實測：topic-hub 的 App prompt 本身約 5.1k 字元，**加上工具清單後是 24.1k 字元** ——
也就是**約 79% 的 system prompt 是工具清單**。

這對 Phase 2 是硬約束：**記憶指示不是在真空裡競爭。** 模型眼前同時攤著 `save_skill`、
`save_workflow`、`create_context_card` 的完整描述，而記憶只是 App prompt 裡的一小段。
§2.4 的邊界規則要能穿透這個比例失衡才算數。

（也因此，任何「離線」評測若只送 App prompt 而不含工具清單，測的是**另一個 prompt**，
結論不可移植。）
...
**DoD / tests.** **同一輸入重複 N 次（N ≥ 6），以通過率驗收**，不是跑一輪看綠燈
（理由見 §4.5.1：失效是機率性的）。每次結果分類成
「寫記憶／走錯機制／沒呼叫工具／捏造／語意反轉」，逐條打掉 §4.5 的四個失效：
...
| **小模型不主動寫記憶** | ❌ **已觀測到**（§4.5，qwen3-14b）—— **機率性**，非每次（§4.5.1） |
| **小模型改去呼叫 `save_skill`** | ❌ **已觀測到**（§4.5）—— 邊界規則必須點名 skill／workflow |
| **小模型寫入時捏造內容** | ❌ **已觀測到**（§4.5）—— 兩條事實 → 十餘項虛構規格 |
| **`user` 型事實在寫入時被語意反轉** | ❌ **已觀測到**（§4.5）—— 「我是 X」→「通知 X」 |
| **記憶指示只佔 system prompt 的一小段** | ⚠️ 約 79% 是工具清單散文（§4.5.2）—— Phase 2 的邊界規則要能穿透這個比例失衡 |
