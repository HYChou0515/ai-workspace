...
- **P10 · KB chat 暫時圖附件 → VLM 描述 → 搜(通用多模態 chat 輸入)—— 要做(用例 B 路 2/3)**
  - **通用擴充**:KB chat 訊息可夾帶**一張暫時上傳圖**(非 ingest、非 SourceDoc)→ 平台在對話當下
    用 VLM 把它描述成查詢文字 → 併入 user 的文字(站點/條件 metadata)→ `kb_search` → agent 綜合作答。
  - 這是「讓 chat 能看一張圖問問題」的通用能力,任何 KB chat 都受惠,非 defect 專屬;它同時是用例 B
    路 2/3 的地基(路 1 = 以圖搜圖,等 P4)。
  - 設計待收斂(P10 grill / TDD 時定):(a) 圖怎麼掛在訊息上(`_MsgBody` 加圖欄 vs 獨立 upload +
    reference);(b) VLM 描述在 turn-level 前處理注入 query,還是 agent 一個通用 `describe_image` tool
    看**這張附件**(非 doc id);(c) 是否複用 workspace chat 既有的 paste/drag 附圖(#364)+ `read_image`
    的 FE/傳輸接縫,避免重造。
  - ⚠️ 交付前 B 準度:主要價值是 triage + 資料累積;細分近似缺陷靠 P4 圖向量。
...
- **P2 / P6 / P7 / P8 / P9 已完成**(皆通用平台擴充,保留)。
- **P10 獨立**,是唯一還要寫的程式碼(通用多模態 chat 輸入);用例 B 路 2/3 靠它。
- **P4** gated on 他隊交付 `ImageEmbedder`(用例 B 路 1)。
- **P5 / P11 零碼**:設定 + guidance + 種子卡。
