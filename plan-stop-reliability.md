...
### P2 補驗（2026-09-08）：指令真的會死，但我證不出是**我的修法**讓它死的

讓假模型回一個 tool call 去跑 `exec(["sleep","300"])`，把真的 `sandbox-host` 起在本機、
app 設成 `kind: http`（**正式環境那條路**，也正是 P2 動刀的地方），然後在瀏覽器按 Stop：

| | `sleep 300` 的下場 |
|---|---|
| 有 P2 修法 | 點擊後 **76ms** 消失 |
| **把修法還原**（`finally: await task`） | 點擊後 **72ms** 消失 |

**對照組沒有紅。** 所以在這個設定下，`task.cancel()` 加不加，指令都會死。最可能的機制是
uvicorn 在 client 斷線時取消了 request task，而那個取消會**沿著 `await task` 傳進去**，
一樣走到 `LocalProcessSandbox.exec` 的 `CancelledError → SIGKILL 整個 process group`。

所以 commit 裡「那條 handler **走不到**」這個說法，**我沒有證據**。修法本身仍然是對的
（明確 cancel 不依賴「取消會不會沿著 await 傳播」，那是隨 ASGI server/版本會變的行為），
但它的效果我在本機重現不出來——**這是我宣稱過頭，不是量測失敗**。

⚠️ 探針的坑：第一次跑報「PROCESS SURVIVED」是假的——`pgrep -af "sleep 300"` 這行**自己**
就含有它要找的字串，永遠匹配到自己，count 到不了 0。要先濾掉自己那行。
...
第七輪之後，Stop 本體（P1–P5）就沒有再被抓到東西了。第八到十輪每一輪最嚴重的發現，都是
**上一輪為了替「起不來的下班窗回合」找一個上界所做的修法**：

| 輪次 | 最嚴重發現 | 我做錯的事 |
|---|---|---|
| 8 | 一次冷 sandbox 就永久 park 掉目標 | 上界掛在**共用計數器** `stall_count` 上，而它的歸零在 `window` 交棒底下，會跨夜累積 |
| 9 | 被配額閘擋掉的那類失敗**完全靜默**，每晚重演 | 我宣稱「thread 上已經有 driver 訊息和錯誤」——只對在 `_send` 內部拋的失敗成立 |
| 10 | 使用者回話一次，就把「今晚放棄」取消掉 | 放棄靠「留著認領」表達，而禮讓那條路無條件 `release`，把它清掉 |

### 最後的形狀

上界裝在 **sweeper 的認領列** `_GoalStretch`（per-stretch、跨 pod、換一晚自動歸零），
連續三次起不來就放過今晚；**完全不碰目標狀態**，明天照常重試，沒有人需要手動解除。
放棄的那一刻寫一則 `role="goal"` marker + 一個通知，**帶真正的錯誤、不叫模型**
（`night_transcript` 會把 `role="error"` 濾掉，所以原本那個 summary 是模型自己編的）。

「今晚已放棄」的判定放在 `try_claim`——那是「今晚能不能起這個 chat」被回答的地方，
所以每一個未來的 `release` 呼叫者都自動涵蓋。
...
- 「放棄整晚 → marker + 鈴」那條路**只有單元測試 + 突變探針**。真 app 裡我試了三種方法都
  沒能讓 `start_offhours_round` 失敗（lazy sandbox 讓死掉的 host 在 preamble 不被碰；
  配額閘在 in-memory 佈署讀到的用量不含塞進去的檔），所以沒有 live 佐證。
  真 app 裡驗到的是：**接線開得起來、sweeper 活著、每個 stretch 只起一次**。
