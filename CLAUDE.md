...
- **`kb_search` vs `ask_knowledge_base` — leaf vs consumer-interface; they don't merge (#270).** `kb_search` is the KB agent's own retrieval *leaf* (vector search → numbered `[n]` passages; needs a `retriever` + `collection_ids` in context). Grant it ONLY to agents that *are* the KB agent: the `kb_chat` presets and the `infer_modules` classifier (likewise `search_wiki`/`read_source` only to the wiki maintainer/reader agents). Every *other* app agent (RCA, playground, topic-hub) gets **`ask_knowledge_base`** instead — it delegates the whole question to a `kb_chat` sub-agent (via the generic `run_subagent` bridge, wired for every app turn in `_send_into` + for workflows — **not** RCA-special; KB-chat turns deliberately omit it so the KB agent can't recurse) and returns a synthesized, cited answer (non-streaming `answer_question`). The point of the sub-agent is **context isolation**: the noisy retrieval (multi-query/HyDE/rerank, raw passages, whole wiki pages) stays in the sub-agent's throwaway context, keeping the consumer's window lean — which matters most for small local models. It still relays the sub-agent's searches + reasoning live into the parent stream via `ctx.on_exec_output` (`answer_question(on_event=…)` + `kb_progress`) so the turn shows progress instead of stalling. **They can't be collapsed into one tool:** `ask_knowledge_base` spawns a KB agent, and that inner agent must search with a *non-delegating* tool (`kb_search`) or it would call `ask_knowledge_base` on itself forever — the leaf is irreducible. So when wiring a new app, list `ask_knowledge_base`, **never** `kb_search`/`search_wiki` (those need a retriever/wiki context the app doesn't have and will fail without). `lookup_glossary` is the exception — a cheap, deterministic, exact-key context-card lookup (no LLM, no retriever), so it's fine to grant any app directly (topic-hub does).
