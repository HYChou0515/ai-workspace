...
def test_the_beam_falls_back_to_the_latest_when_nothing_has_been_graded(tmp_path):
    ...
    history = [
        ...
        Round(version=1, prompt="NEWEST", tune={}, holdout=None),
    ]

    assert pick_parent(history).prompt == "NEWEST"


def test_a_document_whose_probe_reply_is_unusable_costs_only_that_document(tmp_path):
    """One model reply that cannot be parsed must not empty the probe set — the
    set is drawn once and frozen, so a bad reply here is permanent."""
    from workspace_app.kb.graph.tune import ensure_probes

    class _Patchy(ILlm):
        def __init__(self) -> None:
            self.replies = iter(["no json here at all", "{oops not json}", '{"names": ["錫膏"]}'])

        def stream(self, prompt: str) -> Iterator[tuple[str, bool]]:
            yield next(self.replies), False

    holdout = tmp_path / "holdout"
    holdout.mkdir()
    for name in ("a", "b", "c"):
        (holdout / f"{name}.txt").write_text("內容")

    assert ensure_probes(_Patchy(), tmp_path / "rounds", holdout) == ["錫膏"]


def test_a_judge_reply_that_is_json_but_not_verdicts_reports_nothing(tmp_path):
    """Braces are not agreement. A shape that parses but carries no verdict is
    the same non-answer as unparseable text, and must not read as zero."""
    from workspace_app.kb.graph.tune import judge_names
    ...
    assert judge_names(_Wrong("{not: valid, json}"), ["回焊爐"]) == {}
    assert judge_names(_Wrong('{"verdicts": [{"keep": true}]}'), ["回焊爐"]) == {}
...
def test_a_batch_larger_than_the_pool_reads_everything(tmp_path):
    ...
    pool = tmp_path / "tune"
    pool.mkdir()
    (pool / "only.txt").write_text("x")

    assert _batch(pool, 50, seed=0) is None


def test_a_name_the_previous_version_recorded_twice_is_reported_once(tmp_path):
    """A name appears once per document, so anything found in several documents
    arrives as several rows. Repeating it would make one loss look like many and
    push the reviser to over-correct."""
    from workspace_app.kb.graph.tune import lost_names
...
def test_the_history_skips_a_version_that_was_never_scored(tmp_path):
    ...
    (rounds / "v0" / "prompt.txt").write_text("P0")
    (rounds / "v0" / "scorecard.json").write_text(json.dumps({"tune": {}, "holdout": None}))
    (rounds / "v1").mkdir()
    (rounds / "v1" / "prompt.txt").write_text("P1 — written, never graded")

    assert [r.version for r in _history(rounds, upto=1)] == [0]
