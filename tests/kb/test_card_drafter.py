...
def test_parses_a_well_formed_response_into_card_drafts():
    ...
    (card,) = drafter.draft(doc_path="a.md", doc_text="...")
...
def test_tolerates_a_fenced_or_prefixed_response():
    ...
    (card,) = LlmCardDrafter(_FakeLlm(fenced)).draft(doc_path="a.md", doc_text="...")
    assert card.keys == ["M4", "Metal 4"]


def test_an_unparseable_response_yields_no_cards():
    assert (
        LlmCardDrafter(_FakeLlm("I could not find any terms.")).draft(
...
def test_cards_missing_keys_or_with_wrong_types_are_dropped():
    ...
    cards = LlmCardDrafter(_FakeLlm(raw)).draft(doc_path="a.md", doc_text="...")
...
def test_confidence_defaults_to_true_when_absent():
    ...
    (card,) = LlmCardDrafter(_FakeLlm(raw)).draft(doc_path="a.md", doc_text="...")
    assert card.confident is True
...
def test_the_number_of_cards_is_capped():
    ...
    cards = LlmCardDrafter(_FakeLlm(raw), max_cards=3).draft(doc_path="a.md", doc_text="...")
    assert len(cards) == 3


def test_the_prompt_carries_the_document_text_and_path():
    llm = _FakeLlm(_GOOD)
    LlmCardDrafter(llm).draft(doc_path="reflow-spec.md", doc_text="Zone 3 setpoint 245C.")
    (prompt,) = llm.prompts
...
def test_a_cards_value_that_is_not_a_list_yields_nothing():
    assert (
        LlmCardDrafter(_FakeLlm(json.dumps({"cards": "nope"}))).draft(
            doc_path="a.md", doc_text="..."
        )
        == []
    )
...
def test_items_with_a_wrong_typed_field_are_dropped():
    ...
    assert LlmCardDrafter(_FakeLlm(raw)).draft(doc_path="a.md", doc_text="...") == []


def test_an_unterminated_json_object_yields_nothing():
    assert LlmCardDrafter(_FakeLlm('{"cards": [')).draft(doc_path="a.md", doc_text="...") == []
