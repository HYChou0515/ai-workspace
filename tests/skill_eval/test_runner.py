...
def test_the_system_prompt_is_the_apps_own_and_carries_its_three_layers():
    prompt = system_prompt("rca")
    assert "RCA Agent" in prompt  # the app's identity
    assert "One tool call per response" in prompt  # apps/_base.md
    assert "Judge code by running it" in prompt  # apps/_sandbox.md
...
def test_the_skill_body_reaches_the_model_in_the_user_turn(tmp_path):
    ...
    run_scenario(
        chat, Scenario(name="s", prompt="Q?"), tmp_path, skill_name="demo", skill_md=SKILL_MD
    )
...
def test_the_control_arm_asks_the_same_question_with_no_guidance(tmp_path):
    chat = scripted(Turn(content="ok"))
    run_scenario(chat, Scenario(name="s", prompt="Q?"), tmp_path)
    assert chat.seen[0][1]["content"] == "Q?"


def test_an_answer_with_no_tool_calls_ends_the_run(tmp_path):
    t = run_scenario(scripted(Turn(content="42")), Scenario(name="s", prompt="q"), tmp_path)
...
def test_tool_calls_are_executed_and_recorded_in_order(tmp_path):
    ...
    t = run_scenario(chat, Scenario(name="s", prompt="q"), tmp_path)
...
def test_a_model_that_never_answers_stops_at_the_step_limit(tmp_path):
    ...
    t = run_scenario(chat, Scenario(name="s", prompt="q"), tmp_path, max_steps=3)
