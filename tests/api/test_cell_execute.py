"""Phase 9 — POST/DELETE /a/{slug}/items/{id}/notebooks/{path}/cells/{idx}/execute
and POST /a/{slug}/items/{id}/notebooks/{path}/kernel/restart.
...
def test_execute_print_emits_cell_stream_and_done(harness: Harness):
    resp = harness.client.post(
        harness.wpath("/notebooks/nb.ipynb/cells/0/execute"),
        json={"code": "print('hi')"},
    )
...
def test_execute_state_persists_across_cells_in_same_notebook(harness: Harness):
    ...
    resp = harness.client.post(
        harness.wpath("/notebooks/nb.ipynb/cells/1/execute"),
        json={"code": "print(x)"},
    )
    events = _parse_sse(resp.text)
...
def test_execute_different_notebooks_isolated(harness: Harness):
    ...
    harness.client.post(
        harness.wpath("/notebooks/a.ipynb/cells/0/execute"),
        json={"code": "y = 7"},
    )
    resp = harness.client.post(
        harness.wpath("/notebooks/b.ipynb/cells/0/execute"),
        json={"code": "print(y)"},
    )
    events = _parse_sse(resp.text)
    errs = [e for e in events if e["type"] == "cell_error"]
...
def test_execute_nested_notebook_path_supported(harness: Harness):
    ...
    resp = harness.client.post(
        harness.wpath("/notebooks/sub/dir/deep.ipynb/cells/0/execute"),
...
def test_delete_cell_execute_interrupts_returns_204(harness: Harness):
    ...
    resp = harness.client.delete(harness.wpath("/notebooks/nb.ipynb/cells/0/execute"))
...
def test_delete_cell_execute_on_unknown_kernel_returns_204(harness: Harness):
    ...
    resp = harness.client.delete(harness.wpath("/notebooks/missing.ipynb/cells/0/execute"))
    assert resp.status_code == 204
...
def test_post_kernel_restart_returns_204_and_clears_state(harness: Harness):
    ...
    resp = harness.client.post(harness.wpath("/notebooks/nb.ipynb/kernel/restart"))
    assert resp.status_code == 204
...
def test_post_kernel_restart_on_unknown_notebook_returns_204(harness: Harness):
    ...
    resp = harness.client.post(harness.wpath("/notebooks/never.ipynb/kernel/restart"))
    assert resp.status_code == 204
