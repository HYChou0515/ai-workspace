...
def test_create_item_records_activity(harness: Harness):
    harness.client.post("/a/rca/items", json={"title": "Voids spike"})
    feed = harness.client.get("/activity").json()
    assert len(feed) == 1
    assert feed[0]["kind"] == "item_created"
    assert "Voids spike" in feed[0]["text"]
    assert feed[0]["ref"]["item_id"]


def test_close_item_records_activity(harness: Harness):
    item_id = harness.client.post("/a/rca/items", json={"title": "x"}).json()["resource_id"]
    harness.client.post(f"/a/rca/items/{item_id}/close", json={"status": "resolved"})
    feed = harness.client.get("/activity").json()
    kinds = [e["kind"] for e in feed]
    assert "item_closed" in kinds
...
def test_delete_and_move_record_activity(harness: Harness):
    harness.client.put(harness.wpath("/files/a.txt"), content=b"x")
    harness.client.post(harness.wpath("/files/move"), json={"from": "/a.txt", "to": "/b.txt"})
    harness.client.delete(harness.wpath("/files/b.txt"))
...
def test_agent_turn_records_activity(harness: Harness):
    ...
    resp = harness.client.post(harness.wpath("/messages"), json={"content": "hi"})
    assert resp.status_code == 202  # #43: queued; POST awaits the turn then 202
    kinds = [e["kind"] for e in harness.client.get("/activity").json()]
    assert "agent_turn_complete" in kinds


def test_activity_is_newest_first(harness: Harness):
    harness.client.put(harness.wpath("/files/one.txt"), content=b"1")
    harness.client.put(harness.wpath("/files/two.txt"), content=b"2")
    feed = harness.client.get("/activity").json()
