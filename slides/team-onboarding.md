...
| 支柱 | 我們對部門的承諾 | 現在到哪 | 未來要建的（以這一季為例） |
|---|---|---|---|
| ① Harness · 地基 | 下游任務靠平台架構**快速長出新功能，不重複造輪子**。架構要涵蓋未來需求；舊系統嫁接要簡單且**框得住 AI**；要可靠可擴展並**善用下班時間跑 async** | **權限系統 · 知識庫 · 知識圖譜 · 回合引擎 · 沙盒 · 檔案同步**都是現成可用的元件；背景 job 已經會在**晚上與週末**自己跑。新 App 只寫 `app.json` + `model.py` | 把**任何舊系統**都接得上來，讓平台在**沒人看著的時候**也在工作。 **這一季**：接上第一個舊系統、在下班窗跑完第一個長任務 |
| ② 知識 · 沉澱 | 部門知識**保存在系統裡**。不只給 AI 用，也讓**新人與跨部門更快掌握領域知識** | 文件庫 + wiki + 詞彙卡已上線，答案帶 `[n]` 引註點得回原文 | 讓知識從**片段**變成**可推理的關係**，連簡報與截圖裡的也算數。 **這一季**：知識圖譜可巡檢 |
| ③ 橋樑 · oneshot | 領域專家用 AI 產出**可以直接看、直接跑的東西**，取代用文字描述想像的需求文件 | 已可行：md / marp / **ppt**，可附簡單 script 與 sample output | 讓 oneshot 的產出從**看得到**變成**點得動**，並能直接變成需求或任務。 **這一季**：一個業務專案完整跑在平台上 |
| ④ 階梯 · 固化 | **做過一次的事不該做第二次**：一次性操作 → skill / workflow → tool → App → 獨立系統 | PM 是第二個 App：畫 gantt、管 issue，**AI 直接參與 PM 工作** | 讓每一階**自動接上**，一路長到能離開平台的獨立系統。 **這一季**：走通第一個 skill → workflow 的升級 |
...
![w:860](diagrams/team-onboarding-overview.svg)
...
| **specstar** 資料層 | 資料模型、自動 CRUD、索引、向量查詢 | **穩定** — 新模型照既有做法加索引 + migration |
| **背景 worker** | 索引 · wiki · 卡片生成 · 健檢，各自依負載擴縮 | **大致穩定** — **夜間與週末照常執行**；個別 job 的規模仍在調，agent 的**長跑排程**正在架上去 |
...
| 層 | 是什麼 | 它決定什麼 | 誰會改它 |
|---|---|---|---|
| **App** 領域層 | 一個自成一格的儀表板：`app.json` + `model.py` + `prompts/` + `profiles/` | **這個領域的資料長什麼樣**（WorkItem 的欄位與**狀態機**，各 App 資料不互通）、清單 / 看板 / 表單怎麼呈現、agent 的人格與起始內容包，以及功能開關與工具上限 —— **界線怎麼切見附錄 ⑥** | 平台團隊 + 領域負責人 |
...
| **returncode 分配** | 平台既有：`0` 成功 · `2` 參數不合法 · `124` **平台逾時砍掉**（不是你回的）。我們約定其餘非零：`3` 上游拒絕或無權限（**不要重試**）· `4` 暫時失敗（**可以重試**） | 呼叫端只能瞎猜要不要重試 |
| **配合平台的 timeout** | 平台帶**兩個**上限：總時長與閒置。prod 設成**最長 1 小時**、**閒置 60 秒** —— 所以你的工具**每 60 秒至少要吐一行 log 到 stderr** | 超過 60 秒不出聲會被當成 hung 砍掉，回 `124` |
| **env 只放秘密** | 連線資訊與金鑰走 item 的環境變數（派送那次帶入、**不落地**）；一般參數一律走 JSON。**模型看不到 env 是刻意的** —— 所以**不准印進 stdout / stderr，也不准寫進檔案**，除非有強烈理由 | 秘密會跟著 log 和 workspace 一起流出去 |
...
# 下一步
## 這一季要出來的東西 —— 需要誰配合、怎麼算做到了

| 這一季的成果 | 主線 | 需要誰配合 | 怎麼算做到了 |
|---|---|---|---|
| **第一個舊系統接上來** 用 MCP 包成 AI 可呼叫的能力 | ① 連接 | 該系統的 owner；一組**權限可控**的帳號；商定那台 server 由誰維運 | 有人用**一句話**跨系統拿到答案，而且那次查詢走的是**他本人**的權限 |
| **一個業務專案完整跑在平台上** 從 profile + skill 起步，穩定後固化 | ④ 平權 · ② 知識 | 領域專家**一起做第一版**，並願意把「怎麼做」講到能被寫下來 | 領域專家**自己重跑第二次**，不需要我們在旁邊 |
| **會議簡報半自動入庫** 既有的開會 PPT 批次收進知識庫 | ② 知識 | 各單位提供簡報來源，並圈出**哪些可以進、哪些不行** | 上傳一批會議簡報後，問得到**裡面的結論**並**點得回那一頁**，而不是只搜到檔名 |
| **對話當下就能回報對錯** 答錯可以直接糾正 | ② 知識 | 使用者願意順手按 —— 所以必須是**一鍵**，不能是一張表單 | 糾正**會回到知識裡**：同一個問題下一次答得對，而不是只留下一筆評分 |
| **知識圖譜可巡檢** 系統抽出了什麼，攤開來看得見 | ② 知識 | —— | 能**搜尋、篩選、翻頁**地檢視實體與關係，而不是只能相信它 |
| **下班窗跑完第一個長任務** 非上班時段自動續跑 | ③ 自主 | —— | 隔天上班拿到的是**一份收尾摘要**，不是一堆沒人看的日誌 |

### 每一項都有講得出來的驗收樣子 —— 這一季結束時，這六件事**看得到、按得動**。

負責人在會議上認領。第二項的專案清單為機密，口頭補充；本頁只保留可對外說明的打法與配合需求。
