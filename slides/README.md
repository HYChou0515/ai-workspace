...
| 檔案 | 內容 |
|---|---|
| `team-onboarding.md` | 給小組成員的平台導覽。**主檔 5 頁**（使命 / 架構 ①② / 未來 = 要建什麼 / 下一步 = 這一季誰動、怎麼驗收）＋ **附錄 12 頁**（回合資料流、fan-in queue、**固化階梯 → 兩種 tool → 四者的取捨 → App 界線** 四頁連著讀、Sandbox×FileStore、KB 進出、四道安全閘、可靠性、品質關卡） |
| `diagrams/*.mmd` | 圖的**來源**（mermaid） |
| `diagrams/*.svg` | 由 `.mmd` 產生、**deck 實際引用**的圖 |

主檔刻意壓在 5 頁內；細節與決策參考一律進附錄，附錄不限頁數。
