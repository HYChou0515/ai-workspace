...
!!! note "這條路證明的是邏輯，不是環境"
    ...
    工程師的 agent 跑起來時**不是 sandbox**。逾時有（和平台同一個 60 秒），但**沒有輸出上限**，
    也沒有平台注入的環境變數。所以一支在平台上會被截斷輸出的工具，在這裡會整包吐出來。
...
**每一支工具都要有。** 它不只是體積的例外——**沒有它，平台不會執行你的工具**，工程師的
編輯器也不會。把網址給平台團隊不算上架，憑證才算。

流程是：你把工具給平台團隊 review → 他們簽一張給你 → 你存成 repo 根目錄的
`tool-certificate.token`（整行，不要換行）並提交。它會跟著 manifest 一起發布，所以平台驗的
和你 build 時用的是同一張。

憑證上寫著三件事：

- **平台給你的工具名字。** 和你 `[project.scripts]` 取的名字無關，兩邊可以不一樣。因為身分
  來自這裡，所以**別人也叫 `data-fetch` 不會和你衝突**。
- **你的 artifact 住在哪。** 憑證只對這個位置底下的 artifact 有效，所以就算它被複製到別人的
  manifest 也不會過。
- **能多大，以及發布期限。** 見下。

### 體積：預設 150MB

`build-tool` 量的是**壓縮後**的 `tool.tar.gz`——那是每一台機器實際要下載的東西。超過就讓
build 紅，並且列出 bundle 裡最重的幾樣，你不用自己猜。
...
- **為了一張圖帶進整套繪圖庫、為了讀一個欄位帶進整套資料科學堆疊。** 先確認執行時真的用得到。

### 需要更大的額度

跟平台團隊講。他們可以把你的上限調高，但**會附一個發布期限**——那是「你今天先上，但這個
日期前要瘦下來」的意思。

期限過了會發生什麼，講清楚：**你發不出新的超標版本**，build 會紅。但**已經上架的那一版
照常服務**，使用者不受影響。拖延的代價落在你身上，不會落在用你工具的人身上。
