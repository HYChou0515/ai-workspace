...
async def read_file_impl(ctx: RunContextWrapper[AgentToolContext], path: str) -> str:
    ...
    try:
        data = await ctx.context.filestore.read(ctx.context.workspace_id, path)
    except FileNotFound:
...
async def write_file_impl(ctx: RunContextWrapper[AgentToolContext], path: str, content: str) -> str:
    ...
    await ctx.context.filestore.write(ctx.context.workspace_id, path, content.encode("utf-8"))
...
async def ls_impl(ctx: RunContextWrapper[AgentToolContext], prefix: str = "") -> list[str]:
    ...
    return await ctx.context.filestore.ls(ctx.context.workspace_id, prefix)
...
async def exists_impl(ctx: RunContextWrapper[AgentToolContext], path: str) -> bool:
    ...
    return await ctx.context.filestore.exists(ctx.context.workspace_id, path)
...
async def delete_file_impl(ctx: RunContextWrapper[AgentToolContext], path: str) -> str:
    ...
    try:
        await ctx.context.filestore.delete(ctx.context.workspace_id, path)
    except FileNotFound:
        return f"error: file not found: {path}"
