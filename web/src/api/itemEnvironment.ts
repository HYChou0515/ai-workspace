...
export type ItemEnvironment = {
   ...
   *  after the server moved. `Infinity` when the record carries none (an API
   *  pod from before the field, mid-rollout): then nothing is refused as
   *  over here and the server's 422 still stands, as it did before. */
  maxCpuCores: number;
...
export const itemEnvironmentApi: ItemEnvironmentApi = {
  async get(slug, itemId) {
    ...
    return {
      ...
      maxCpuCores: w.max_cpu_cores ?? Number.POSITIVE_INFINITY,
      maxMemoryBytes: w.max_memory_bytes ?? Number.POSITIVE_INFINITY,
    };
  },
