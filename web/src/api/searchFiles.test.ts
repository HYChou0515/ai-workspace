...
describe("searchFiles / replaceInFiles (mock client)", () => {
  ...
  beforeEach(async () => {
    const created = await mockApi.createInvestigation({ title: "search test" });
    inv = created.resource_id;
    await mockApi.writeFile(inv, "/a.md", "void rate spiked\nall good\nVOID again");
    await mockApi.writeFile(inv, "/b.txt", "nothing here");
    await mockApi.writeFile(inv, "/data/x.csv", "void in csv");
  });

  it("finds substring matches across files, case-insensitive by default", async () => {
    const res = await mockApi.searchFiles(inv, "void");
