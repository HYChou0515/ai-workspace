...
describe("EntryView — show_file (files the agent showed)", () => {
  ...
  it("a user message shows the markings it was sent with, a refused one too (#847 P7)", () => {
    render(
      <EntryView
        entry={{
          kind: "message",
    ...
    const chips = screen.getAllByTestId("marking-chip");
    expect(chips.map((c) => c.getAttribute("data-refused"))).toEqual(["false", "true"]);
    expect(screen.getByTestId("message-body")).toHaveTextContent(/^why\?$/);
  });
