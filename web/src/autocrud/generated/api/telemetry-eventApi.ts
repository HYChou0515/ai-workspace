...
const BASE = '/telemetry-event';

export const telemetryEventApi = {
  create: (data: TelemetryEvent) =>
    client.post<RevisionInfo>(BASE, data),

  list: (params?: SearchParams & { returns?: string }) =>
    client.get<FullResource<TelemetryEvent>[]>(BASE, { params }),
  ...
  get: (id: string, params?: { revision_id?: string; partial?: string[]; returns?: string; include_deleted?: boolean }) =>
    client.get<FullResource<TelemetryEvent>>(`${BASE}/${id}`, { params }),

  update: (id: string, data: TelemetryEvent, params?: { change_status?: string; mode?: string }) =>
