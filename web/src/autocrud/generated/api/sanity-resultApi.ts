...
const BASE = '/sanity-result';

export const sanityResultApi = {
  create: (data: SanityResult) =>
    client.post<RevisionInfo>(BASE, data),

  list: (params?: SearchParams & { returns?: string }) =>
    client.get<FullResource<SanityResult>[]>(BASE, { params }),
  ...
  get: (id: string, params?: { revision_id?: string; partial?: string[]; returns?: string; include_deleted?: boolean }) =>
    client.get<FullResource<SanityResult>>(`${BASE}/${id}`, { params }),

  update: (id: string, data: SanityResult, params?: { change_status?: string; mode?: string }) =>
